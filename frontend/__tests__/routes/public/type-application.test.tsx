...
vi.mock('~/.server/routes/helpers/apply-route-helpers', () => ({ loadApplyState: vi.fn().mockReturnValue({ id: '123', typeOfApplication: 'delegate' }), saveApplyState: vi.fn().mockReturnValue({ headers: { 'Set-Cookie': 'some-set-cookie-header' } }) }));
...
describe('_public.apply.id.type-of-application', () => {
  ...
  describe('action()', () => {
    it('should validate missing applcation type selection', async () => {
      ...
      expect(response).toEqual({ data: { errors: { typeOfApplication: 'apply:type-of-application.error-message.type-of-application-required' } }, init: { status: 400 }, type: 'DataWithResponseInit' });
    });
