...
export const handle = {
  ...
  pageIdentifier: pageIds.public.apply.child.exitApplication,
...
export async function loader({ context: { session }, params, request }: LoaderFunctionArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('apply-child:exit-application.page-title') }) };
  ...
  return (
    <div className="max-w-prose">
      <div className="mb-8 space-y-4">
        <p>{t('apply-child:exit-application.are-you-sure')}</p>
        <p>{t('apply-child:exit-application.click-back')}</p>
      </div>
      <fetcher.Form method="post" noValidate className="flex flex-wrap items-center gap-3">
        <input type="hidden" name="_csrf" value={csrfToken} />
        <ButtonLink id="back-button" routeId="$lang/_public/apply/$id/child/review-adult-information" params={params} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form:Back - Exiting the application click">
          <FontAwesomeIcon icon={faChevronLeft} className="me-3 block size-4" />
          {t('apply-child:exit-application.back-btn')}
        </ButtonLink>
        <Button variant="primary" disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form:Exit - Exiting the application click">
