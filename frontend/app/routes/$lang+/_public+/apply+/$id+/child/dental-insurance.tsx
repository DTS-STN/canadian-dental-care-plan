...
export const handle = {
  ...
  pageTitleI18nKey: 'apply-child:dental-insurance.title',
};
...
export async function loader({ context: { session }, params, request }: LoaderFunctionArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('apply-child:dental-insurance.title') }) };

  return json({ id: state, csrfToken, meta, defaultState: state.childState.dentalInsurance, editMode: state.childState.editMode });
}
...
export async function action({ context: { session }, params, request }: ActionFunctionArgs) {
  ...
  const dentalInsuranceSchema: z.ZodType<DentalInsuranceState> = z.boolean({ errorMap: () => ({ message: t('apply-child:dental-insurance.error-message.dental-insurance-required') }) });
  ...
  saveApplyChildState({ params, request, session, state: { dentalInsurance: parsedDataResult.data } });
        ...
        <fetcher.Form method="post" noValidate>
          ...
          {editMode ? (
            ...
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              ...
              <ButtonLink
                id="back-button"
                routeId="$lang+/_public+/apply+/$id+/child/communication-preference"
                params={params}
                disabled={isSubmitting}
