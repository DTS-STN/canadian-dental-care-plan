...
export async function action({ context: { session }, params, request }: ActionFunctionArgs) {
  ...
  const personalInformationSchema = z
    .object({
      ...
      mailingAddress: z.string().trim().min(1, t('apply-child:personal-information.error-message.address-required')).max(30),
      mailingApartment: z.string().trim().max(30).optional(),
      mailingCountry: z.string().trim().min(1, t('apply-child:personal-information.error-message.country-required')),
      mailingProvince: z.string().trim().min(1, t('apply-child:personal-information.error-message.province-required')).optional(),
      mailingCity: z.string().trim().min(1, t('apply-child:personal-information.error-message.city-required')).max(100),
    ...
    .superRefine((val, ctx) => {
      ...
      if (val.mailingCountry === CANADA_COUNTRY_ID || val.mailingCountry === USA_COUNTRY_ID) {
        if (!val.mailingProvince || validator.isEmpty(val.mailingProvince)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:personal-information.error-message.province-required'), path: ['mailingProvince'] });
        }

        if (!val.mailingPostalCode || validator.isEmpty(val.mailingPostalCode)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:personal-information.error-message.postal-code-required'), path: ['mailingPostalCode'] });
        } else if (!isValidPostalCode(val.mailingCountry, val.mailingPostalCode)) {
          const message = val.mailingCountry === CANADA_COUNTRY_ID ? t('apply-child:personal-information.error-message.postal-code-valid') : t('apply-child:personal-information.error-message.zip-code-valid');
      ...
      if (val.copyMailingAddress === false) {
        if (!val.homeAddress || validator.isEmpty(val.homeAddress)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:personal-information.error-message.address-required'), path: ['homeAddress'] });
        }

        if (!val.homeCountry || validator.isEmpty(val.homeCountry)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:personal-information.error-message.country-required'), path: ['homeCountry'] });
        }

        if (!val.homeCity || validator.isEmpty(val.homeCity)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:personal-information.error-message.city-required'), path: ['homeCity'] });
        }
        ...
        if (val.homeCountry === CANADA_COUNTRY_ID || val.homeCountry === USA_COUNTRY_ID) {
          if (!val.homeProvince || validator.isEmpty(val.homeProvince)) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:personal-information.error-message.province-required'), path: ['homeProvince'] });
          }

          if (!val.homePostalCode || validator.isEmpty(val.homePostalCode)) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:personal-information.error-message.postal-code-required'), path: ['homePostalCode'] });
          } else if (!isValidPostalCode(val.homeCountry, val.homePostalCode)) {
            const message = val.homeCountry === CANADA_COUNTRY_ID ? t('apply-child:personal-information.error-message.postal-code-valid') : t('apply-child:personal-information.error-message.zip-code-valid');
    ...
    <>
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate aria-describedby="form-instructions-info form-instructions">
          ...
          <fieldset>
            <legend className="sr-only">{t('apply-child:personal-information.mailing-address.header')}</legend>
          ...
          <fieldset>
            <legend className="sr-only">{t('apply-child:personal-information.home-address.header')}</legend>
