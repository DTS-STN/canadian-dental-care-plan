...
export async function loader({ context: { session }, params, request }: LoaderFunctionArgs) {
  ...
  return json({
    ...
    preferredLanguages,
    defaultState: {
      ...(state.childState.communicationPreferences ?? {}),
      email: state.childState.communicationPreferences?.email ?? state.childState.personalInformation?.email,
...
export async function action({ context: { session }, params, request }: ActionFunctionArgs) {
  ...
  saveApplyChildState({ params, request, session, state: { communicationPreferences: parsedDataResult.data } });
      ...
      append: preferredMethodValue === communicationMethodEmail.id && (
        <div className="mb-6 grid items-end gap-6 md:grid-cols-2">
          <p className="md:col-span-2" id="email-note">
            {t('apply-child:communication-preference.email-note')}
          </p>
          <InputField
            id="email"
            type="email"
    ...
    <>
      ...
      <div className="max-w-prose">
        ...
        <p className="mb-6" id="form-instructions-note">
          {t('apply-child:communication-preference.note')}
        </p>
        ...
        <fetcher.Form method="post" noValidate aria-describedby="form-instructions form-instructions-note">
          ...
          {editMode ? (
            ...
            <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              ...
              <ButtonLink
                id="back-button"
                routeId="$lang+/_public+/apply+/$id+/child/personal-information"
                params={params}
                disabled={isSubmitting}
