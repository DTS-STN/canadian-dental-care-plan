    ...
    .superRefine((val, ctx) => {
      if (val.hasFederalBenefits) {
        if (!val.federalSocialProgram || validator.isEmpty(val.federalSocialProgram)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:dental-benefits.error-message.program-required'), path: ['federalSocialProgram'] });
        }
      }
    })
    .transform((val) => {
    ...
    .superRefine((val, ctx) => {
      if (val.hasProvincialTerritorialBenefits) {
        if (!val.province || validator.isEmpty(val.province)) {
          ...
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-child:dental-benefits.error-message.program-required'), path: ['provincialTerritorialSocialProgram'] });
        }
      }
    })
    .transform((val) => {
        ...
        <fetcher.Form method="post" noValidate aria-describedby="access-to-benefits-note eligibility-note form-instructions">
          ...
          <section>
            <p className="mb-4" id="access-to-benefits-note">
              {t('apply-child:dental-benefits.access-to-dental')}
            </p>
            <p className="mb-4" id="eligibility-note">
              {t('apply-child:dental-benefits.eligibility-criteria')}
            </p>
