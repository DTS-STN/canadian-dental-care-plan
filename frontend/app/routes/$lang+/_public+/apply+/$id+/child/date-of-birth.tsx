...
export const handle = {
  ...
  pageTitleI18nKey: 'apply-child:eligibility.date-of-birth.page-title',
...
export async function action({ context: { session }, params, request }: ActionFunctionArgs) {
  ...
  const dateOfBirthSchema = z
    .object({
      dateOfBirthYear: z
        .number({
          required_error: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-year-required'),
          invalid_type_error: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-year-number'),
        })
        .int()
        .positive(),
      dateOfBirthMonth: z
        .number({
          required_error: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-month-required'),
        })
        .int()
        .positive(),
      dateOfBirthDay: z
        .number({
          required_error: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-day-required'),
          invalid_type_error: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-day-number'),
        })
        .int()
        .positive(),
    ...
    .superRefine((val, ctx) => {
      ...
      if (!isValid(parsedDateOfBirth)) {
        ctx.addIssue({
          ...
          message: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-valid'),
          path: ['dateOfBirth'],
        });
      } else if (!isPast(parsedDateOfBirth)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-is-past'),
          path: ['dateOfBirth'],
        });
      } else if (differenceInYears(new Date(), parsedDateOfBirth) > 150) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: t('apply-child:eligibility.date-of-birth.error-message.date-of-birth-is-past-valid'),
          path: ['dateOfBirth'],
        });
      }
    })
  ...
  return (
    <>
      ...
      <div className="max-w-prose">
        <p className="mb-6">{t('apply-child:eligibility.date-of-birth.description')}</p>
        <p className="mb-2 italic">{t('apply:required-label')}</p>
        {errorSummaryItems.length > 0 && <ErrorSummary id={errorSummaryId} errors={errorSummaryItems} />}
        <fetcher.Form method="post" noValidate>
          <input type="hidden" name="_csrf" value={csrfToken} />
          <div className="mb-6 space-y-4">
            <h2 className="text-xl font-bold">{t('apply-child:eligibility.date-of-birth.age-heading')}</h2>
            <DatePickerField
              id="date-of-birth"
              names={{
                day: 'dateOfBirthDay',
              ...
              legend={t('apply-child:eligibility.date-of-birth.form-instructions')}
              errorMessages={{
          ...
          {editMode ? (
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Button variant="primary" id="continue-button" disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form:Save - Child flow date of birth click">
                {t('apply-child:eligibility.date-of-birth.save-btn')}
              </Button>
              <ButtonLink
                id="back-button"
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form:Cancel - Child flow date of birth click"
              >
                {t('apply-child:eligibility.date-of-birth.cancel-btn')}
              </ButtonLink>
            </div>
          ) : (
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <Button variant="primary" id="continue-button" disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form:Continue - Child flow date of birth click">
                {t('apply-child:eligibility.date-of-birth.continue-btn')}
                <FontAwesomeIcon icon={isSubmitting ? faSpinner : faChevronRight} className={cn('ms-3 block size-4', isSubmitting && 'animate-spin')} />
              </Button>
              <ButtonLink id="back-button" routeId="$lang+/_public+/apply+/$id+/child/tax-filing" params={params} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form:Back - Child flow date of birth click">
                <FontAwesomeIcon icon={faChevronLeft} className="me-3 block size-4" />
                {t('apply-child:eligibility.date-of-birth.back-btn')}
              </ButtonLink>
            </div>
          )}
        </fetcher.Form>
