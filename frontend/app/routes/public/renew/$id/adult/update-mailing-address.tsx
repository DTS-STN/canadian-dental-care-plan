...
export async function action({ context: { appContainer, session }, params, request }: ActionFunctionArgs) {
  ...
  const mailingAddressSchema = z
    .object({
      ...
      city: z.string().trim().max(100).refine(isAllValidInputCharacters, t('renew-adult:update-address.error-message.characters-valid')),
      postalCode: z.string().trim().max(100).refine(isAllValidInputCharacters, t('renew-adult:update-address.error-message.characters-valid')).optional(),
    })
    .superRefine((val, ctx) => {
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <fieldset className="mb-6">
            <legend className="mb-4 font-lato text-2xl font-bold">{t('renew-adult:update-address.mailing-address.header')}</legend>
