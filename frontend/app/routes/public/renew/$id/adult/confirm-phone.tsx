  ...
  const phoneNumberSchema = z
    .object({
      isNewOrUpdatedPhoneNumber: z.enum(ADD_OR_UPDATE_PHONE_OPTION, {
        error: t('renew-adult:confirm-phone.error-message.add-or-update-required'),
      }),
    ...
    .superRefine((val, ctx) => {
      if (val.isNewOrUpdatedPhoneNumber === ADD_OR_UPDATE_PHONE_OPTION.yes && !val.phoneNumber) {
        ctx.addIssue({ code: 'custom', message: t('renew-adult:confirm-phone.error-message.phone-required'), path: ['phoneNumber'] });
      }
    })
    .transform((val) => ({
      ...val,
