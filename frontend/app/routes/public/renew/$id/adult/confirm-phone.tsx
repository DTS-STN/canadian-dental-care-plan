  ...
  const phoneNumberSchema = z
    .object({
      ...
      phoneNumber: z
        ...
        .trim()
        .max(100)
        .refine((val) => !val || (val.replace(/[^\d]/g, '').length <= 11 ? isValidPhoneNumber(val, 'CA') : true), t('renew-adult:confirm-phone.error-message.phone-number-valid'))
        .refine((val) => !val || (val.replace(/[^\d]/g, '').length > 11 ? isValidPhoneNumber(val) : true), t('renew-adult:confirm-phone.error-message.phone-number-valid-international'))
        .optional(),
      phoneNumberAlt: z
        .string()
        .trim()
        .max(100)
        .refine((val) => !val || (val.replace(/[^\d]/g, '').length <= 11 ? isValidPhoneNumber(val, 'CA') : true), t('renew-adult:confirm-phone.error-message.phone-number-alt-valid'))
        .refine((val) => !val || (val.replace(/[^\d]/g, '').length > 11 ? isValidPhoneNumber(val) : true), t('renew-adult:confirm-phone.error-message.phone-number-alt-valid-international'))
        .optional(),
    })
    .superRefine((val, ctx) => {
