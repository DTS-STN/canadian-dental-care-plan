...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const emailSchema = z
    .object({
      email: z
        .string({ error: t('renew-adult:confirm-email.error-message.email-required') })
        .trim()
        .min(1)
        .max(64),
    })
    ...
    .superRefine((val, ctx) => {
      if (!validator.isEmail(val.email)) {
        ctx.addIssue({ code: 'custom', message: t('renew-adult:confirm-email.error-message.email-valid'), path: ['email'] });
      }
    });
    ...
    <>
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <p className="mb-4">{t('renew-adult:confirm-email.provide-email')}</p>
          <p className="mb-8">{t('renew-adult:confirm-email.verify-email')}</p>
          <p className="mb-4 italic">{t('renew:required-label')}</p>
          <div className="mb-6">
            <InputField id="email" name="email" type="email" inputMode="email" className="w-full" autoComplete="email" defaultValue={defaultState} errorMessage={errors?.email} label={t('renew-adult:confirm-email.email-legend')} maxLength={64} required />
          </div>
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              <Button variant="primary" id="save-button" disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult:Save - Email click">
              ...
              <ButtonLink
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult:Cancel - Email click"
              >
            ...
            <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton variant="primary" id="continue-button" loading={isSubmitting} endIcon={faChevronRight} data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult:Continue - Email click">
                {t('renew-adult:confirm-email.continue')}
              </LoadingButton>
              <ButtonLink
                id="back-button"
                variant="secondary"
                routeId="public/renew/$id/adult/communication-preference"
                params={params}
                disabled={isSubmitting}
                startIcon={faChevronLeft}
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult:Back - Email click"
              >
                {t('renew-adult:confirm-email.back')}
              </ButtonLink>
            </div>
          )}
        </fetcher.Form>
