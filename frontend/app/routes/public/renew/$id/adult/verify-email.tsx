...
export const handle = {
  ...
  pageIdentifier: pageIds.public.renew.adult.verifyEmail,
  pageTitleI18nKey: 'renew-adult:verify-email.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('renew-adult:verify-email.page-title') }) };
  ...
  if (formAction === FORM_ACTION.submit) {
    const verificationCodeSchema = z.object({
      verificationCode: z
        .string()
        .trim()
        .min(1, t('renew-adult:verify-email.error-message.verification-code-required'))
        .transform(extractDigits)

        .superRefine((val, ctx) => {
          if (state.verifyEmail && state.verifyEmail.verificationAttempts >= MAX_ATTEMPTS) {
            ctx.addIssue({ code: 'custom', message: t('renew-adult:verify-email.error-message.verification-code-max-attempts'), path: ['verificationCode'] });
          }
        }),
    });
  ...
  return (
    <>
      ...
      <div className="max-w-prose">
        <ErrorAlert>
          <h2 className="mb-2 font-bold">{t('renew-adult:verify-email.verification-code-alert.heading')}</h2>
          <p className="mb-2">{t('renew-adult:verify-email.verification-code-alert.detail')}</p>
        </ErrorAlert>
          ...
          <fieldset className="mb-6">
            ...
            <div className="grid items-end gap-6 md:grid-cols-2">
              <InputField
                ...
                label={t('renew-adult:verify-email.verification-code-label')}
            ...
            >
              {t('renew-adult:verify-email.request-new-code')}
            </LoadingButton>
          </fieldset>
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult:Save - Verify email click"
              >
              ...
              <ButtonLink
                ...
                params={params}
                disabled={isSubmitting}
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult:Cancel - Verify email click"
              >
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('renew-adult:verify-email.code-sent.heading')}</DialogTitle>
            </DialogHeader>
