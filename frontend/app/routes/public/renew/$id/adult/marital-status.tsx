...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const maritalStatusSchema = z.object({
    maritalStatus: z
      .string({ error: t('renew-adult:marital-status.error-message.marital-status-required') })
      .trim()
  ...
  const partnerInformationSchema = z.object({
    ...
    yearOfBirth: z
      ...
      .refine((year) => Number.parseInt(year) > currentYear - 150, t('renew-adult:marital-status.error-message.yob-is-past'))
      .refine((year) => Number.parseInt(year) < currentYear, t('renew-adult:marital-status.error-message.yob-is-future')),
    socialInsuranceNumber: z
      ...
      .superRefine((sin, ctx) => {
        if (!isValidSin(sin)) {
          ctx.addIssue({ code: 'custom', message: t('renew-adult:marital-status.error-message.sin-valid') });
        } else if (
          ...
          ctx.addIssue({ code: 'custom', message: t('renew-adult:marital-status.error-message.sin-unique') });
        }
      }),
...
export default function RenewAdultMaritalStatus({ loaderData, params }: Route.ComponentProps) {
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <div className="mb-8 space-y-6">
            ...
            {(marriedOrCommonlaw === MARITAL_STATUS.commonlaw || marriedOrCommonlaw === MARITAL_STATUS.married) && (
              <>
                <h2 className="font-lato mb-6 text-2xl font-bold">{t('renew-adult:marital-status.spouse-or-commonlaw')}</h2>
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <LoadingButton id="cancel-button" variant="secondary" name="_action" value={FORM_ACTION.cancel} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult:Cancel - Marital status click">
