...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const state = loadRenewItaStateForReview({ params, request, session });
  const benefitRenewalDto = appContainer.get(TYPES.BenefitRenewalStateMapper).mapRenewItaStateToItaBenefitRenewalDto(state);
  await appContainer.get(TYPES.BenefitRenewalService).createItaBenefitRenewal(benefitRenewalDto);
