...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  instrumentationService.countHttpStatus('public.renew.adult-child.confirm-marital-status', 200);
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  if (!parsedDataResult.success) {
    instrumentationService.countHttpStatus('public.renew.adult-child.confirm-marital-status', 400);
  ...
  instrumentationService.countHttpStatus('public.renew.adult-child.confirm-marital-status', 302);
...
export default function RenewAdultChildConfirmMaritalStatus({ loaderData, params }: Route.ComponentProps) {
        ...
        <fetcher.Form method="post" noValidate>
          ...
          {editMode ? (
            ...
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton variant="primary" id="continue-button" loading={isSubmitting} endIcon={faChevronRight} data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult_Child:Continue - Marital status click">
