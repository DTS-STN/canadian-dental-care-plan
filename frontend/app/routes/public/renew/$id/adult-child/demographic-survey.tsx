  ...
  const demographicSurveySchema = z
    ...
    .superRefine((val, ctx) => {
      if (val.indigenousStatus === IS_APPLICANT_FIRST_NATIONS_YES_OPTION.toString() && !val.firstNations) {
        ctx.addIssue({ code: 'custom', message: t('renew-adult-child:demographic-survey.error-message.first-nations-required'), path: ['firstNations'] });
      }

      if (val.ethnicGroups.includes(ANOTHER_ETHNIC_GROUP_OPTION.toString()) && !val.anotherEthnicGroup) {
        ctx.addIssue({ code: 'custom', message: t('renew-adult-child:demographic-survey.error-message.another-ethnic-group-required'), path: ['anotherEthnicGroup'] });
      }
    });
...
export default function RenewAdultChildDemographicSurveyQuestions({ loaderData, params }: Route.ComponentProps) {
  ...
  const indigenousStatusOptions = indigenousStatuses.map((status) => ({
    ...
    append: status.id === IS_APPLICANT_FIRST_NATIONS_YES_OPTION.toString() && isIndigenousStatusValue && (
      <InputRadios id="first-nations" name="firstNations" legend={t('renew-adult-child:demographic-survey.indigenous-status')} options={firstNationsOptions} errorMessage={errors?.firstNations} required />
    ),
  }));
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <div className="mb-8 space-y-6">
            ...
            <InputRadios
              ...
              legend={t('renew-adult-child:demographic-survey.disability-status')}
              options={disabilityStatusOptions}
              errorMessage={errors?.disabilityStatus}
              required
              helpMessagePrimary={t('renew-adult-child:demographic-survey.disability-help-message')}
            />
            ...
            <InputRadios
              id="gender-status"
              name="genderStatus"
              legend={t('renew-adult-child:demographic-survey.gender-status')}
              options={genderStatusOptions}
              errorMessage={errors?.genderStatus}
              required
              helpMessagePrimary={t('renew-adult-child:demographic-survey.gender-help-message')}
            />
          </div>
          ...
          {editMode ? (
            <div className="mt-8 flex flex-wrap items-center gap-3">
              ...
              <ButtonLink
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult_Child:Cancel - Voluntary demographic questions click"
              >
            ...
            <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Adult_Child:Continue - Voluntary demographic questions click"
              >
