...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  instrumentationService.countHttpStatus('public.renew.child.exit-application', 200);
  return { meta };
}
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  instrumentationService.countHttpStatus('public.renew.child.exit-application', 302);
  return redirect(t('renew-child:exit-application.exit-link'));
}

export default function RenewChildExitApplication({ loaderData, params }: Route.ComponentProps) {
