...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = {
    title: t('gcweb:meta.title.template', { title: t('renew-child:children.information.page-title', { childName }) }),
    dcTermsTitle: t('gcweb:meta.title.template', { title: t('renew-child:children.information.page-title', { childName }) }),
  };

  instrumentationService.countHttpStatus('public.renew.child.children.information', 200);
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const childInformationSchema = z
    .object({
      firstName: z
        .string()
        .trim()
        .min(1, t('renew-child:children.information.error-message.first-name-required'))
        .max(100)
        .refine(isAllValidInputCharacters, t('renew-child:children.information.error-message.characters-valid'))
        .refine((firstName) => !hasDigits(firstName), t('renew-child:children.information.error-message.first-name-no-digits')),
      lastName: z
        .string()
        .trim()
        .min(1, t('renew-child:children.information.error-message.last-name-required'))
        .max(100)
        .refine(isAllValidInputCharacters, t('renew-child:children.information.error-message.characters-valid'))
  ...
  if (!parsedDataResult.success) {
    instrumentationService.countHttpStatus('public.renew.child.children.information', 400);
  ...
  if (matches && !matches.includes(true)) {
    instrumentationService.countHttpStatus('public.renew.child.children.information.child-not-found', 200);
  ...
  instrumentationService.countHttpStatus('public.renew.child.children.information', 302);
  ...
  if (!parsedDataResult.data.isParent) {
    return redirect(getPathById('public/renew/$id/child/children/$childId/parent-or-guardian', params));
  }
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <div className="mb-8 space-y-6">
            <Collapsible id="name-instructions" summary={t('renew-child:children.information.single-legal-name')}>
              <p>{t('renew-child:children.information.name-instructions')}</p>
            </Collapsible>
            <div className="grid items-end gap-6 md:grid-cols-2">
              <InputSanitizeField
                id="first-name"
                name="firstName"
                label={t('renew-child:children.information.first-name')}
                className="w-full"
                maxLength={100}
              ...
              <InputSanitizeField
                id="last-name"
                name="lastName"
                label={t('renew-child:children.information.last-name')}
                className="w-full"
                maxLength={100}
            ...
            <DatePickerField
              ...
              legend={t('renew-child:children.information.date-of-birth')}
              errorMessages={{
            ...
            <InputPatternField
              id="client-number"
              name="clientNumber"
              label={t('renew-child:children.information.client-number')}
              inputMode="numeric"
              format={renewalCodeInputPatternFormat}
              helpMessagePrimary={t('renew-child:children.information.client-number-detail')}
          ...
          {editMode ? (
            ...
            <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton id="continue-button" variant="primary" loading={isSubmitting} endIcon={faChevronRight} data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Child:Continue - Child information click">
                {t('renew-child:children.information.continue-btn')}
              </LoadingButton>
              <ButtonLink
                id="back-button"
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Child:Back - Child information click"
              >
                {t('renew-child:children.information.back-btn')}
              </ButtonLink>
            </div>
          )}
        </fetcher.Form>
