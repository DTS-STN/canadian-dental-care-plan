...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  instrumentationService.countHttpStatus('public.renew.child.confirm-email', 200);
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  if (!parsedDataResult.success) {
    instrumentationService.countHttpStatus('public.renew.child.confirm-email', 400);
    return data({ errors: transformFlattenedError(parsedDataResult.error.flatten()) }, { status: 400 });
  }

  saveRenewState({ params, session, state: { contactInformation: { ...state.contactInformation, ...parsedDataResult.data } } });

  instrumentationService.countHttpStatus('public.renew.child.confirm-email', 302);
          ...
          {editMode ? (
            ...
            <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Child:Continue - Email click"
              >
                {t('renew-child:confirm-email.continue-btn')}
              </LoadingButton>
              <ButtonLink id="back-button" routeId="public/renew/$id/child/confirm-phone" params={params} disabled={isSubmitting} startIcon={faChevronLeft} data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Child:Back - Email click">
