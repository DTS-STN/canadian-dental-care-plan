...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const emailSchema = z
    .object({
      email: z
        .string({ error: t('renew-child:confirm-email.error-message.email-required') })
        .trim()
        .min(1)
        .max(64),
    })
    ...
    .superRefine((val, ctx) => {
      if (!validator.isEmail(val.email)) {
        ctx.addIssue({ code: 'custom', message: t('renew-child:confirm-email.error-message.email-valid'), path: ['email'] });
      }
    });
    ...
    <>
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <p className="mb-4">{t('renew-child:confirm-email.provide-email')}</p>
          <p className="mb-8">{t('renew-child:confirm-email.verify-email')}</p>
          <p className="mb-4 italic">{t('renew:required-label')}</p>
          <div className="mb-6 grid items-end gap-6 md:grid-cols-2">
            <InputField id="email" name="email" type="email" inputMode="email" className="w-full" autoComplete="email" defaultValue={defaultState} errorMessage={errors?.email} label={t('renew-child:confirm-email.email-legend')} maxLength={64} required />
          </div>
          {editMode ? (
            ...
            <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton variant="primary" id="continue-button" loading={isSubmitting} endIcon={faChevronRight} data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Child:Continue - Email click">
                {t('renew-child:confirm-email.continue')}
              </LoadingButton>
              <ButtonLink
                id="back-button"
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Renew Application Form-Child:Back - Email click"
              >
                {t('renew-child:confirm-email.back')}
              </ButtonLink>
            </div>
          )}
        </fetcher.Form>
