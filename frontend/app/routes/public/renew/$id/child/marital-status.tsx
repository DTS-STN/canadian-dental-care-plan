...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const maritalStatusSchema = z.object({
    maritalStatus: z
      .string({ error: t('renew-child:marital-status.error-message.marital-status-required') })
      .trim()
  ...
  const partnerInformationSchema = z.object({
    ...
    socialInsuranceNumber: z
      ...
      .superRefine((sin, ctx) => {
        if (!isValidSin(sin)) {
          ctx.addIssue({ code: 'custom', message: t('renew-child:marital-status.error-message.sin-valid') });
        } else if (
          ...
          ctx.addIssue({ code: 'custom', message: t('renew-child:marital-status.error-message.sin-unique') });
        }
      }),
