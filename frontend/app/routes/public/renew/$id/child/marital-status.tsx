...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  if (!state.hasMaritalStatusChanged) {
    instrumentationService.countHttpStatus('public.renew.child.confirm-marital-status', 302);
  ...
  instrumentationService.countHttpStatus('public.renew.child.confirm-marital-status', 200);
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  if (!parsedMaritalStatus.success || (renewStateHasPartner(parsedMaritalStatus.data.maritalStatus) && !parsedPartnerInformation.success)) {
    instrumentationService.countHttpStatus('public.renew.child.confirm-marital-status', 400);
    return data(
      {
        errors: {
