...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  instrumentationService.countHttpStatus('public.renew.child.confirm-phone', 200);
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const phoneNumberSchema = z
    .object({
      ...
      phoneNumber: phoneSchema({
        ...
        invalid_phone_international_error: t('renew-child:confirm-phone.error-message.phone-number-valid-international'),
      }).optional(),
      phoneNumberAlt: phoneSchema({
        invalid_phone_canadian_error: t('renew-child:confirm-phone.error-message.phone-number-alt-valid'),
  ...
  if (!parsedDataResult.success) {
    instrumentationService.countHttpStatus('public.renew.child.confirm-phone', 400);
    return data({ errors: transformFlattenedError(parsedDataResult.error.flatten()) }, { status: 400 });
  }

  saveRenewState({ params, session, state: { contactInformation: { ...state.contactInformation, ...parsedDataResult.data } } });

  instrumentationService.countHttpStatus('public.renew.child.confirm-phone', 302);
...
export default function RenewChildConfirmPhone({ loaderData, params }: Route.ComponentProps) {
