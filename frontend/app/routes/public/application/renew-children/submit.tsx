...
export async function action({ context: { appContainer, session }, request, params }: Route.ActionArgs) {
  ...
  const submitTermsSchema = z
    .object({
      acknowledgeInfo: z.string().trim().nonempty(t('application-renew-child:submit.error-message.acknowledge-info-required')),
      acknowledgeCriteria: z.string().trim().nonempty(t('application-renew-child:submit.error-message.acknowledge-criteria-required')),
    })
    .transform((val) => ({
