...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const dentalBenefits = await publicApplicationStateResolver.resolveDentalBenefitsValue({ dentalBenefits: state.dentalBenefits }, locale);
    ...
    <>
      ...
      <div className="max-w-prose space-y-10">
        ...
        <section>
          ...
          <p className="mt-4">
            <Trans ns="applicationFullAdult" i18nKey={($) => $.confirm.cdcpChecker} components={{ cdcpLink, noWrap: <span className="whitespace-nowrap" /> }} />
          </p>
        ...
        <section>
          ...
          <p className="mt-4">
            <Trans ns="applicationFullAdult" i18nKey={($) => $.confirm.getUpdatesText} components={{ mscaLinkAccount }} />
          </p>
        ...
        <div className="my-6">
          <div className="px-12 print:hidden">
            <PrintButton size="lg" variant="primary" errorMessage={t(($) => $.confirm.printUnavailable)} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Full_Adult:Print bottom - Application successfully submitted click">
              {t(($) => $.confirm.printBtn)}
            </PrintButton>
          </div>
        </div>
