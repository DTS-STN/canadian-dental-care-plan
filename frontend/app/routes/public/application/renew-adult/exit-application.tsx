...
export const handle = {
  ...
  pageIdentifier: pageIds.public.application.renewAdult.exitApplication,
  pageTitleI18nKey: 'application-renew-adult:exit-application.page-title',
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  return redirect(t('application-renew-adult:exit-application.exit-link'));
}
  ...
  return (
    <div className="max-w-prose">
      <div className="mb-8 space-y-4">
        <p>{t('application-renew-adult:exit-application.are-you-sure')}</p>
        <p>{t('application-renew-adult:exit-application.click-back')}</p>
      </div>
      <fetcher.Form method="post" noValidate className="flex flex-wrap items-center gap-3">
        ...
        <LoadingButton variant="primary" loading={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Renewal Form-Adult:Exit - Exiting the application click">
          {t('application-renew-adult:exit-application.exit-btn')}
        </LoadingButton>
      </fetcher.Form>
    </div>
  );
