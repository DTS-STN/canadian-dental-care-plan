...
export async function loader({ context: { appContainer, session }, request, params }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('application-simplified-family:childrens-application.page-title') }) };
    ...
    <>
      ...
      <div className="max-w-prose space-y-8">
        {state.children.map((child, index) => {
          ...
          return (
            <div key={child.id}>
              <h2 className="font-lato mb-4 text-2xl font-bold">{t('application-simplified-family:childrens-application.child-title', { childNumber: index + 1 })}</h2>
              ...
              <Card className="my-2">
                <CardHeader>
                  <CardTitle>{t('application-simplified-family:childrens-application.child-information-card-title', { childNumber: index + 1 })}</CardTitle>
              ...
              <Card className="my-2">
                <CardHeader>
                  <CardTitle>{t('application-simplified-family:childrens-application.child-dental-insurance-card-title')}</CardTitle>
                ...
                <CardContent>
                  {child.dentalInsurance === undefined ? (
                    <p>{t('application-simplified-family:childrens-application.child-dental-insurance-indicate-status')}</p>
                  ) : (
                    <DefinitionList layout="single-column">
              ...
              <Card className="my-2">
                ...
                <CardFooter className="border-t bg-zinc-100">
                  <ButtonLink
                    ...
                    size="lg"
                  >
                    {child.dentalBenefits === undefined ? t('application-simplified-family:childrens-application.add-answer') : t('application-simplified-family:childrens-application.edit-child-dental-benefits', { childNumber: index + 1 })}
                  </ButtonLink>
                </CardFooter>
              </Card>
        ...
        <fetcher.Form method="post" onSubmit={handleSubmit} noValidate>
          ...
          <Button variant="primary" id="add-child" name="_action" value={FORM_ACTION.add} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Renewal Form-Family:Add child - Child(ren) application click">
            {t('application-simplified-family:childrens-application.add-child')}
          </Button>
        </fetcher.Form>

        <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
          <NavigationButtonLink disabled={!allChildrenCompleted} variant="primary" direction="next" routeId="public/application/$id/simplified-family/submit" params={params}>
            {t('application-simplified-family:childrens-application.submit-btn')}
          </NavigationButtonLink>
          <NavigationButtonLink variant="secondary" direction="previous" routeId="public/application/$id/simplified-family/parent-or-guardian" params={params}>
            {t('application-simplified-family:childrens-application.back-btn')}
          </NavigationButtonLink>
        </div>
      </div>
    </>
