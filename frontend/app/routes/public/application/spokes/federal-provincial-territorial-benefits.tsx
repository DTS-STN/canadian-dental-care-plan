...
export const handle = {
  ...
  pageTitleI18nKey: 'application-spokes:dentalBenefits.title',
};
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('application-spokes:dentalBenefits.title') }) };
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const federalBenefitsSchema = z
    .object({
      hasFederalBenefits: z.boolean({ error: t('application-spokes:dentalBenefits.errorMessage.federalBenefitRequired') }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasFederalBenefits && (!val.federalSocialProgram || validator.isEmpty(val.federalSocialProgram))) {
        ctx.addIssue({ code: 'custom', message: t('application-spokes:dentalBenefits.errorMessage.federalBenefitProgramRequired'), path: ['federalSocialProgram'] });
      }
    })
    .transform((val) => {
      return {
  ...
  const provincialTerritorialBenefitsSchema = z
    .object({
      hasProvincialTerritorialBenefits: z.boolean({ error: t('application-spokes:dentalBenefits.errorMessage.provincialBenefitRequired') }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasProvincialTerritorialBenefits) {
        if (!val.province || validator.isEmpty(val.province)) {
          ctx.addIssue({ code: 'custom', message: t('application-spokes:dentalBenefits.errorMessage.provincialTerritorialRequired'), path: ['province'] });
        } else if (!val.provincialTerritorialSocialProgram || validator.isEmpty(val.provincialTerritorialSocialProgram)) {
          ctx.addIssue({ code: 'custom', message: t('application-spokes:dentalBenefits.errorMessage.provincialBenefitProgramRequired'), path: ['provincialTerritorialSocialProgram'] });
        }
      }
    })
    .transform((val) => {
  ...
  return (
    <div className="max-w-prose">
      <ErrorSummaryProvider actionData={fetcher.data}>
        <p className="mb-4">{t('application-spokes:dentalBenefits.accessToDental')}</p>
        <p className="mb-4">{t('application-spokes:dentalBenefits.eligibilityCriteria')}</p>
        <p className="mb-4 italic">{t('application:requiredLabel')}</p>
        <ErrorSummary />
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <fieldset className="mb-6">
            <legend className="font-lato mb-4 text-2xl font-bold">{t('application-spokes:dentalBenefits.federalBenefits.title')}</legend>
            <InputRadios
              id="has-federal-benefits"
              name="hasFederalBenefits"
              legend={t('application-spokes:dentalBenefits.federalBenefits.legend')}
              options={[
                {
                  children: <Trans ns={handle.i18nNamespaces} i18nKey="application-spokes:dentalBenefits.federalBenefits.optionYes" />,
                  ...
                  append: hasFederalBenefitValue === true && (
                    <InputRadios
                      ...
                      legend={t('application-spokes:dentalBenefits.federalBenefits.socialPrograms.legend')}
                ...
                {
                  children: <Trans ns={handle.i18nNamespaces} i18nKey="application-spokes:dentalBenefits.federalBenefits.optionNo" />,
          ...
          <fieldset className="mb-8">
            <legend className="font-lato mb-4 text-2xl font-bold">{t('application-spokes:dentalBenefits.provincialTerritorialBenefits.title')}</legend>
            <InputRadios
              id="has-provincial-territorial-benefits"
              name="hasProvincialTerritorialBenefits"
              legend={t('application-spokes:dentalBenefits.provincialTerritorialBenefits.legend')}
              options={[
                {
                  children: <Trans ns={handle.i18nNamespaces} i18nKey="application-spokes:dentalBenefits.provincialTerritorialBenefits.optionYes" />,
                  ...
                  append: hasProvincialTerritorialBenefitValue === true && (
                    <div className="space-y-6">
                      <InputSelect
                        ...
                        label={t('application-spokes:dentalBenefits.provincialTerritorialBenefits.socialPrograms.inputLegend')}
                        onChange={handleOnRegionChanged}
                        options={[
                          {
                            children: t('application-spokes:dentalBenefits.selectOne'),
                            value: '',
                            hidden: true,
                          },
                      ...
                      {provinceValue && (
                        <InputRadios
                          ...
                          legend={t('application-spokes:dentalBenefits.provincialTerritorialBenefits.socialPrograms.radioLegend')}
                          legendClassName="font-normal"
                ...
                {
                  children: <Trans ns={handle.i18nNamespaces} i18nKey="application-spokes:dentalBenefits.provincialTerritorialBenefits.optionNo" />,
          ...
          <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
            <LoadingButton variant="primary" id="save-button" loading={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Spoke:Save - Access to other dental benefits click">
              {t('application-spokes:dentalBenefits.saveBtn')}
            </LoadingButton>
            <ButtonLink
              id="back-button"
              variant="secondary"
            ...
            >
              {t('application-spokes:dentalBenefits.backBtn')}
            </ButtonLink>
          </div>
        </fetcher.Form>
