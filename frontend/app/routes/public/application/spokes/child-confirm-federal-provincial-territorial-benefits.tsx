...
import type { Route } from './+types/child-confirm-federal-provincial-territorial-benefits';
...
export const handle = {
  ...
  pageIdentifier: pageIds.public.application.spokes.childConfirmFederalProvincialTerritorialBenefits,
  pageTitleI18nKey: 'application-spokes:children.confirm-dental-benefits.title',
};
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = {
    title: t('gcweb:meta.title.template', { title: t('application-spokes:children.confirm-dental-benefits.title', { childName }) }),
    dcTermsTitle: t('gcweb:meta.title.template', { title: t('application-spokes:children.confirm-dental-benefits.title', { childName: childNumber }) }),
  };
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const dentalBenefitsChangedSchema = z.object({
    hasFederalProvincialTerritorialBenefits: z.boolean({ error: t('application-spokes:children.confirm-dental-benefits.error-message.federal-provincial-territorial-benefit-required') }),
  });
...
export default function SpokeChildConfirmFederalProvincialTerritorialBenefits({ loaderData, params }: Route.ComponentProps) {
  ...
  const { childName, defaultState, typeAndFlow } = loaderData;

  const fetcher = useFetcher<typeof action>();
  const isSubmitting = fetcher.state !== 'idle';
  const [federalProvincialTerritorialBenefitChangedValue, setFederalProvincialTerritorialBenefitChangedValue] = useState(defaultState);
  ...
  function handleOnFederalProvincialTerritorialBenefitChanged(e: React.ChangeEvent<HTMLInputElement>) {
    setFederalProvincialTerritorialBenefitChangedValue(e.target.value === FEDERAL_BENEFITS_CHANGED_OPTION.yes);
  }

  return (
    <div className="max-w-prose">
      <p className="mb-4">{t('application-spokes:children.confirm-dental-benefits.access-to-dental')}</p>
      <p className="mb-4">{t('application-spokes:children.confirm-dental-benefits.eligibility-criteria')}</p>
      ...
      <fetcher.Form method="post" noValidate>
        ...
        <fieldset className="mb-6">
          <InputRadios
            ...
            legend={t('application-spokes:children.confirm-dental-benefits.has-benefits', { childName })}
            options={[
              {
                children: <Trans ns={handle.i18nNamespaces} i18nKey="application-spokes:children.confirm-dental-benefits.option-yes" />,
              ...
              {
                children: <Trans ns={handle.i18nNamespaces} i18nKey="application-spokes:children.confirm-dental-benefits.option-no" />,
