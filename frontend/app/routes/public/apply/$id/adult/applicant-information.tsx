  ...
  const applicantInformationSchema = z
    .object({
      socialInsuranceNumber: z
        ...
        .superRefine((sin, ctx) => {
          if (!isValidSin(sin)) {
            ctx.addIssue({ code: 'custom', message: t('apply-adult:applicant-information.error-message.sin-valid') });
          } else if (state.partnerInformation && formatSin(sin) === formatSin(state.partnerInformation.socialInsuranceNumber)) {
            ctx.addIssue({ code: 'custom', message: t('apply-adult:applicant-information.error-message.sin-unique') });
          }
        }),
      firstName: z
        .string()
        ...
        <fetcher.Form method="post" noValidate>
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button id="cancel-button" name="_action" variant="secondary" value={FORM_ACTION.cancel} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Adult:Cancel - Applicant information click">
