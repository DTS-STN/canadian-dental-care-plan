...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const emailSchema = z
    .object({
      email: z
        .string({ error: t('apply-adult:email.error-message.email-required') })
        .trim()
        .min(1)
        .max(64),
    })
    ...
    .superRefine((val, ctx) => {
      if (!validator.isEmail(val.email)) {
        ctx.addIssue({ code: 'custom', message: t('apply-adult:email.error-message.email-valid'), path: ['email'] });
      }
    });
  ...
  return (
    <>
      <div className="my-6 sm:my-8">
        <Progress value={73} size="lg" label={t('apply:progress.label')} />
      </div>
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <div className="mb-6 grid items-end gap-6 md:grid-cols-2">
            <InputField id="email" name="email" type="email" inputMode="email" className="w-full" autoComplete="email" defaultValue={defaultState} errorMessage={errors?.email} label={t('apply-adult:email.email-legend')} maxLength={64} required />
          </div>
          {editMode ? (
