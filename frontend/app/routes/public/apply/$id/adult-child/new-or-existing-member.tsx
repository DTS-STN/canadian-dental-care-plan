...
export const handle = {
  ...
  pageTitleI18nKey: 'apply-adult-child:new-or-existing-member.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('apply-adult-child:new-or-existing-member.page-title') }) };
  ...
  const newOrExistingMemberSchema = z
    .object({
      newOrExistingMember: z.enum(NEW_OR_EXISTING_MEMBER_OPTION, {
        error: t('apply-adult-child:new-or-existing-member.error-message.is-new-or-existing-member-required'),
      }),
      clientNumber: z
        .string()
        .trim()
    ...
    .superRefine((val, ctx) => {
      if (val.newOrExistingMember === NEW_OR_EXISTING_MEMBER_OPTION.yes) {
        if (!val.clientNumber) {
          ctx.addIssue({
            code: 'custom',
            message: t('apply-adult-child:new-or-existing-member.error-message.client-number-required'),
            path: ['clientNumber'],
          });
        } else if (!isValidClientNumberRenewal(val.clientNumber)) {
          ctx.addIssue({
            code: 'custom',
            message: t('apply-adult-child:new-or-existing-member.error-message.client-number-valid'),
            path: ['clientNumber'],
          });
        }
      }
...
export default function ApplyFlowNewOrExistingMember({ loaderData, params }: Route.ComponentProps) {
  ...
  const options: InputRadiosProps['options'] = [
    {
      children: <Trans ns={handle.i18nNamespaces} i18nKey="apply-adult-child:new-or-existing-member.yes" components={{ bold: <strong /> }} />,
    ...
    {
      children: <Trans ns={handle.i18nNamespaces} i18nKey="apply-adult-child:new-or-existing-member.no" components={{ bold: <strong /> }} />,
    ...
    <>
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <InputRadios id="new-or-existing-member" name="newOrExistingMember" legend={t('apply-adult-child:new-or-existing-member.previously-enrolled')} options={options} errorMessage={errors?.newOrExistingMember} required />
          {isNewOrExistingMember && (
            <div className="my-8">
              <InputPatternField
                ...
                label={t('apply-adult-child:new-or-existing-member.client-number-label')}
                inputMode="numeric"
          ...
          {editMode ? (
            <div className="mt-8 flex flex-wrap items-center gap-3">
              ...
              <Button id="cancel-button" name="_action" variant="secondary" value={FORM_ACTION.cancel} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Adult_Child:Cancel - New or existing member click">
            ...
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              ...
              <ButtonLink
                id="back-button"
                variant="secondary"
                routeId={userAgeCategory === 'youth' ? 'public/apply/$id/adult-child/living-independently' : 'public/apply/$id/adult-child/applicant-information'}
                params={params}
