...
export const handle = {
  ...
  pageIdentifier: pageIds.public.apply.adultChild.verifyEmail,
  pageTitleI18nKey: 'apply-adult-child:verify-email.page-title',
  ...
  if (formAction === FORM_ACTION.submit) {
    const verificationCodeSchema = z.object({
      verificationCode: z
        .string()
        .trim()
        .min(1, t('apply-adult-child:verify-email.error-message.verification-code-required'))
        .transform(extractDigits)
        .superRefine((val, ctx) => {
          if (state.verifyEmail && state.verifyEmail.verificationAttempts >= MAX_ATTEMPTS) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('apply-adult-child:verify-email.error-message.verification-code-max-attempts'), path: ['verificationCode'] });
          }
        }),
    });
...
export default function ApplyFlowVerifyEmail({ loaderData, params }: Route.ComponentProps) {
  ...
  const communicationLink = <InlineLink routeId="public/apply/$id/adult-child/communication-preference" params={params} />;
  ...
  return (
    <>
      ...
      <div className="max-w-prose">
        <ErrorAlert>
          <h2 className="mb-2 font-bold">{t('apply-adult-child:verify-email.verification-code-alert.heading')}</h2>
          <p className="mb-2">{t('apply-adult-child:verify-email.verification-code-alert.detail')}</p>
        </ErrorAlert>
        <errorSummary.ErrorSummary />
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <fieldset className="mb-6">
            <p className="mb-4">{t('apply-adult-child:verify-email.verification-code', { email })}</p>
            <p className="mb-4">{t('apply-adult-child:verify-email.request-new')}</p>
            <p className="mb-8">
              <Trans ns={handle.i18nNamespaces} i18nKey="apply-adult-child:verify-email.unable-to-verify" components={{ communicationLink }} />
            </p>
            ...
            <div className="grid items-end gap-6 md:grid-cols-2">
              <InputField
                ...
                label={t('apply-adult-child:verify-email.verification-code-label')}
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button id="cancel-button" name="_action" value={FORM_ACTION.cancel} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Adult_Child:Cancel - Verify email click">
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('apply-adult-child:verify-email.code-sent.heading')}</DialogTitle>
            </DialogHeader>
            <DialogDescription>{t('apply-adult-child:verify-email.code-sent.detail', { email })}</DialogDescription>
            <DialogFooter>
              <DialogClose asChild>
