...
export const handle = {
  ...
  pageIdentifier: pageIds.public.apply.adultChild.maritalStatus,
  pageTitleI18nKey: 'apply-adult-child:marital-status.page-title',
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const maritalStatusSchema = z.object({
    maritalStatus: z
      .string({ error: t('apply-adult-child:marital-status.error-message.marital-status-required') })
      .trim()
  ...
  const partnerInformationSchema = z.object({
    ...
    socialInsuranceNumber: z
      ...
      .superRefine((sin, ctx) => {
        if (!isValidSin(sin)) {
          ctx.addIssue({ code: 'custom', message: t('apply-adult-child:marital-status.error-message.sin-valid') });
        } else if (
          ...
          ctx.addIssue({ code: 'custom', message: t('apply-adult-child:marital-status.error-message.sin-unique') });
        }
      }),
    ...
    return [
      {
        ...
        children: t('apply-adult-child:marital-status.single'),
        defaultChecked: defaultState.maritalStatus === MARITAL_STATUS.single,
        onChange: handleChange,
      },
      {
        value: MARITAL_STATUS.married,
        children: t('apply-adult-child:marital-status.married'),
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <div className="mb-8 space-y-6">
            <InputRadios id="marital-status" name="maritalStatus" legend={t('apply-adult-child:marital-status.marital-status')} options={maritalStatusOptions} errorMessage={errors?.maritalStatus} required />

            {(marriedOrCommonlaw === MARITAL_STATUS.commonlaw || marriedOrCommonlaw === MARITAL_STATUS.married) && (
              <>
                <h2 className="font-lato mb-6 text-2xl font-bold">{t('apply-adult-child:marital-status.spouse-or-commonlaw')}</h2>
                <p className="mb-4">{t('apply-adult-child:marital-status.provide-sin')}</p>
                <p className="mb-6">{t('apply-adult-child:marital-status.required-information')}</p>
                <InputPatternField
                  ...
                  inputMode="numeric"
                  helpMessagePrimary={t('apply-adult-child:marital-status.help-message.sin')}
                ...
                <InputCheckbox id="confirm" name="confirm" value="yes" errorMessage={errors?.confirm} defaultChecked={defaultState.confirm === true} required>
                  {t('apply-adult-child:marital-status.confirm-checkbox')}
                </InputCheckbox>
              </>
            )}
          </div>
