...
export const handle = {
  ...
  pageIdentifier: pageIds.public.apply.child.childFederalProvincialTerritorialBenefits,
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const federalBenefitsSchema = z
    .object({
      hasFederalBenefits: z.boolean({ error: t('apply-child:children.dental-benefits.error-message.federal-benefit-required') }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasFederalBenefits && (!val.federalSocialProgram || validator.isEmpty(val.federalSocialProgram))) {
        ctx.addIssue({ code: 'custom', message: t('apply-child:children.dental-benefits.error-message.federal-benefit-program-required'), path: ['federalSocialProgram'] });
      }
    })
    .transform((val) => {
      return {
  ...
  const provincialTerritorialBenefitsSchema = z
    .object({
      hasProvincialTerritorialBenefits: z.boolean({ error: t('apply-child:children.dental-benefits.error-message.provincial-benefit-required') }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasProvincialTerritorialBenefits) {
        if (!val.province || validator.isEmpty(val.province)) {
          ctx.addIssue({ code: 'custom', message: t('apply-child:children.dental-benefits.error-message.provincial-territorial-required'), path: ['province'] });
        } else if (!val.provincialTerritorialSocialProgram || validator.isEmpty(val.provincialTerritorialSocialProgram)) {
          ctx.addIssue({ code: 'custom', message: t('apply-child:children.dental-benefits.error-message.provincial-benefit-program-required'), path: ['provincialTerritorialSocialProgram'] });
        }
      }
    })
    .transform((val) => {
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <fieldset className="mb-6">
            <legend className="font-lato mb-4 text-2xl font-bold">{t('apply-child:children.dental-benefits.federal-benefits.title', { childName })}</legend>
            <InputRadios
          ...
          <fieldset className="mb-8">
            <legend className="font-lato mb-4 text-2xl font-bold">{t('apply-child:children.dental-benefits.provincial-territorial-benefits.title')}</legend>
            <InputRadios
          ...
          {editMode ? (
            ...
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              ...
              <ButtonLink
                id="back-button"
                routeId="public/apply/$id/child/children/$childId/confirm-federal-provincial-territorial-benefits"
                params={params}
                disabled={isSubmitting}
