...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const emailSchema = z
    .object({
      email: z
        .string({ error: t('apply-child:email.error-message.email-required') })
        .trim()
        .min(1)
        .max(64),
    })
    ...
    .superRefine((val, ctx) => {
      if (!validator.isEmail(val.email)) {
        ctx.addIssue({ code: 'custom', message: t('apply-child:email.error-message.email-valid'), path: ['email'] });
      }
    });
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <ButtonLink
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Child:Cancel - Email click"
              >
