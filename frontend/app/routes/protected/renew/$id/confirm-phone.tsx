...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-renew:confirm-phone.page-title') }) };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.renew.confirm-phone', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.renew.confirm-phone', { userId: idToken.sub });
...
export default function ProtectedRenewConfirmPhone({ loaderData, params }: Route.ComponentProps) {
  ...
  return (
    <div className="max-w-prose">
      <p className="mb-4 italic">{t('protected-renew:all-optional-label')}</p>
