...
import type { AddressInformationState } from '~/route-helpers/protected-renew-route-helpers.server';
...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.renew.updateAddress,
  pageTitleI18nKey: 'protected-renew:update-address.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: LoaderFunctionArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-renew:update-address.page-title') }) };
...
export async function action({ context: { appContainer, session }, params, request }: ActionFunctionArgs) {
  ...
  const addressInformationSchema = z
    .object({
      mailingAddress: z.string().trim().min(1, t('protected-renew:update-address.error-message.mailing-address.address-required')).max(30).refine(isAllValidInputCharacters, t('protected-renew:update-address.error-message.characters-valid')),
      mailingCountry: z.string().trim().min(1, t('protected-renew:update-address.error-message.mailing-address.country-required')),
      mailingProvince: z.string().trim().min(1, t('protected-renew:update-address.error-message.mailing-address.province-required')).optional(),
      mailingCity: z.string().trim().min(1, t('protected-renew:update-address.error-message.mailing-address.city-required')).max(100).refine(isAllValidInputCharacters, t('protected-renew:update-address.error-message.characters-valid')),
      mailingPostalCode: z.string().trim().max(100).refine(isAllValidInputCharacters, t('protected-renew:update-address.error-message.characters-valid')).optional(),
      copyMailingAddress: z.boolean(),
      homeAddress: z.string().trim().max(30).refine(isAllValidInputCharacters, t('protected-renew:update-address.error-message.characters-valid')).optional(),
      homeCountry: z.string().trim().optional(),
      homeProvince: z.string().trim().optional(),
      homeCity: z.string().trim().max(100).refine(isAllValidInputCharacters, t('protected-renew:update-address.error-message.characters-valid')).optional(),
      homePostalCode: z.string().trim().max(100).refine(isAllValidInputCharacters, t('protected-renew:update-address.error-message.characters-valid')).optional(),
    })
    .superRefine((val, ctx) => {
      if (val.mailingCountry === CANADA_COUNTRY_ID || val.mailingCountry === USA_COUNTRY_ID) {
        if (!val.mailingProvince || validator.isEmpty(val.mailingProvince)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.mailing-address.province-required'), path: ['mailingProvince'] });
        }
        if (!val.mailingPostalCode || validator.isEmpty(val.mailingPostalCode)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.mailing-address.postal-code-required'), path: ['mailingPostalCode'] });
        } else if (!isValidPostalCode(val.mailingCountry, val.mailingPostalCode)) {
          const message = val.mailingCountry === CANADA_COUNTRY_ID ? t('protected-renew:update-address.error-message.mailing-address.postal-code-valid') : t('protected-renew:update-address.error-message.mailing-address.zip-code-valid');
          ...
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.mailing-address.invalid-postal-code-for-province'), path: ['mailingPostalCode'] });
        }
      }
      ...
      if (val.mailingCountry && val.mailingCountry !== CANADA_COUNTRY_ID && val.mailingPostalCode && isValidPostalCode(CANADA_COUNTRY_ID, val.mailingPostalCode)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.mailing-address.invalid-postal-code-for-country'), path: ['mailingCountry'] });
      }

      if (val.copyMailingAddress === false) {
        if (!val.homeAddress || validator.isEmpty(val.homeAddress)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.home-address.address-required'), path: ['homeAddress'] });
        }

        if (!val.homeCountry || validator.isEmpty(val.homeCountry)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.home-address.country-required'), path: ['homeCountry'] });
        }

        if (!val.homeCity || validator.isEmpty(val.homeCity)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.home-address.city-required'), path: ['homeCity'] });
        }
        ...
        if (val.homeCountry === CANADA_COUNTRY_ID || val.homeCountry === USA_COUNTRY_ID) {
          if (!val.homeProvince || validator.isEmpty(val.homeProvince)) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.home-address.province-required'), path: ['homeProvince'] });
          }
          if (!val.homePostalCode || validator.isEmpty(val.homePostalCode)) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.home-address.postal-code-required'), path: ['homePostalCode'] });
          } else if (!isValidPostalCode(val.homeCountry, val.homePostalCode)) {
            const message = val.homeCountry === CANADA_COUNTRY_ID ? t('protected-renew:update-address.error-message.home-address.postal-code-valid') : t('protected-renew:update-address.error-message.home-address.zip-code-valid');
            ...
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.home-address.invalid-postal-code-for-province'), path: ['homePostalCode'] });
          }
        }
        ...
        if (val.homeCountry && val.homeCountry !== CANADA_COUNTRY_ID && val.homePostalCode && isValidPostalCode(CANADA_COUNTRY_ID, val.homePostalCode)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-renew:update-address.error-message.home-address.invalid-postal-code-for-country'), path: ['homeCountry'] });
        }
      }
    })
    .transform((val) => ({
  ...
  saveProtectedRenewState({ params, session, state: { addressInformation: updatedData } });
...
export default function ProtectedRenewUpdateAddress() {
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <fieldset className="mb-6">
            <legend className="mb-4 font-lato text-2xl font-bold">{t('protected-renew:update-address.mailing-address.header')}</legend>
          ...
          <fieldset className="mb-8">
            <legend className="mb-4 font-lato text-2xl font-bold">{t('protected-renew:update-address.home-address.header')}</legend>
            <div className="space-y-6">
