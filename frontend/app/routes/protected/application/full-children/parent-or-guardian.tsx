...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.application.fullChild.parentOrGuardian,
  pageTitleI18nKey: 'protected-application-full-child:parent-or-guardian.page-heading',
...
export async function loader({ context: { appContainer, session }, request, params }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-full-child:parent-or-guardian.page-title') }) };
  ...
  return (
    <>
      ...
      <div className="max-w-prose space-y-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('protected-application-full-child:parent-or-guardian.marital-status')}</CardTitle>
            <CardAction>{sections.maritalStatus.completed && <StatusTag status="complete" />}</CardAction>
          </CardHeader>
          <CardContent>
            {state.maritalStatus === undefined ? (
              <p>{t('protected-application-full-child:parent-or-guardian.select-your-status')}</p>
            ) : (
              <DefinitionList layout="single-column">
                <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.marital-status')}>
                  <p>{state.maritalStatus.name}</p>
                </DefinitionListItem>
                {state.partnerInformation && (
                  <>
                    <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.spouse-sin')}>
                      <p>{formatSin(state.partnerInformation.socialInsuranceNumber)}</p>
                    </DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.spouse-yob')}>
                      <p>{state.partnerInformation.yearOfBirth}</p>
                    </DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.consent')}>
                      {state.partnerInformation.confirm ? t('protected-application-full-child:parent-or-guardian.consent-yes') : t('protected-application-full-child:parent-or-guardian.consent-no')}
                    </DefinitionListItem>
                  </>
                )}
              </DefinitionList>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('protected-application-full-child:parent-or-guardian.phone-number')}</CardTitle>
            <CardAction>{sections.phoneNumber.completed && <StatusTag status="complete" />}</CardAction>
          </CardHeader>
          <CardContent>
            {state.phoneNumber?.hasChanged ? (
              <DefinitionList layout="single-column">
                <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.phone-number')}>
                  <p>{state.phoneNumber.value.primary}</p>
                </DefinitionListItem>
                {state.phoneNumber.value.alternate && (
                  <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.alt-phone-number')}>
                    <p>{state.phoneNumber.value.alternate}</p>
                  </DefinitionListItem>
                )}
              </DefinitionList>
            ) : (
              <p>{t('protected-application-full-child:parent-or-guardian.phone-number-help')}</p>
            )}
          </CardContent>
          <CardFooter className="border-t bg-zinc-100">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('protected-application-full-child:parent-or-guardian.mailing-and-home-address')}</CardTitle>
          ...
          <CardContent>
            {mailingAddressInfo === undefined || homeAddressInfo === undefined ? (
              <p>{t('protected-application-full-child:parent-or-guardian.address-help')}</p>
            ) : (
              <DefinitionList layout="single-column">
                <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.mailing-address')}>
                  <Address
                    address={{
                ...
                <DefinitionListItem term={t('protected-application-full-child:parent-or-guardian.home-address')}>
                  <Address
                    address={{
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('protected-application-full-child:parent-or-guardian.communication-preferences')}</CardTitle>
        ...
        <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
          <NavigationButtonLink disabled={!allSectionsCompleted} variant="primary" direction="next" routeId="protected/application/$id/full-children/childrens-application" params={params}>
            {t('protected-application-full-child:parent-or-guardian.childrens-application')}
          ...
          <NavigationButtonLink variant="secondary" direction="previous" routeId="protected/application/$id/type-of-application" params={params}>
            {t('protected-application-full-child:parent-or-guardian.type-of-application')}
          </NavigationButtonLink>
        </div>
      </div>
    </>
