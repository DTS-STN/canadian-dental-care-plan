...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.application.newAdult.contactInformation,
    ...
    <>
      ...
      <div className="max-w-prose space-y-8">
          ...
          <CardContent>
            {state.phoneNumber?.hasChanged ? (
              <DefinitionList layout="single-column">
                <DefinitionListItem term={t('protected-application-full-adult:contact-information.phone-number')}>{state.phoneNumber.value.primary}</DefinitionListItem>
                {state.phoneNumber.value.alternate && <DefinitionListItem term={t('protected-application-full-adult:contact-information.alt-phone-number')}>{state.phoneNumber.value.alternate}</DefinitionListItem>}
              </DefinitionList>
            ) : (
              <p>{t('protected-application-full-adult:contact-information.phone-number-help')}</p>
            )}
          </CardContent>
          <CardFooter className="border-t bg-zinc-100">
            <ButtonLink id="edit-phone-number-button" variant="link" className="p-0" routeId="protected/application/$id/phone-number" params={params} startIcon={sections.phoneNumber.completed ? faPenToSquare : faCirclePlus} size="lg">
        ...
        <Card>
          ...
          <CardContent>
            {mailingAddressInfo === undefined || homeAddressInfo === undefined ? (
              <p>{t('protected-application-full-adult:contact-information.address-help')}</p>
            ) : (
              <DefinitionList layout="single-column">
        ...
        <Card>
          ...
          <CardContent>
            {state.communicationPreferences?.hasChanged ? (
              <DefinitionList layout="single-column">
                <DefinitionListItem term={t('protected-application-full-adult:contact-information.preferred-language')}>{preferredLanguage?.name}</DefinitionListItem>
                <DefinitionListItem term={t('protected-application-full-adult:contact-information.preferred-method')}>{preferredMethod?.name}</DefinitionListItem>
                <DefinitionListItem term={t('protected-application-full-adult:contact-information.preferred-notification-method')}>{preferredNotificationMethod?.name}</DefinitionListItem>
                {state.email && <DefinitionListItem term={t('protected-application-full-adult:contact-information.email')}>{state.email}</DefinitionListItem>}
              </DefinitionList>
            ) : (
              <p>{t('protected-application-full-adult:contact-information.communication-preferences-help')}</p>
            )}
          </CardContent>
          <CardFooter className="border-t bg-zinc-100">
            <ButtonLink
              id="edit-communication-preferences-button"
              variant="link"
              className="p-0"
        ...
        <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
          ...
          <NavigationButtonLink variant="secondary" direction="previous" routeId="protected/application/$id/full-adult/marital-status" params={params}>
