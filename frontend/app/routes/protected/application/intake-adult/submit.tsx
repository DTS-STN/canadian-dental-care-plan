...
export const handle = {
  ...
  pageTitleI18nKey: 'protected-application-intake-adult:submit.page-heading',
...
export async function loader({ context: { appContainer, session }, request, params }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-intake-adult:submit.page-title') }) };
...
export async function action({ context: { appContainer, session }, request, params }: Route.ActionArgs) {
  ...
  const submitTermsSchema = z.object({
    acknowledgeInfo: z.literal(true, { error: t('protected-application-intake-adult:submit.error-message.acknowledge-info-required') }),
    acknowledgeCriteria: z.literal(true, { error: t('protected-application-intake-adult:submit.error-message.acknowledge-criteria-required') }),
  });
  ...
  return redirect(getPathById('protected/application/$id/intake-adult/confirmation', params));
}
...
export default function NewAdultSubmit({ loaderData, params }: Route.ComponentProps) {
  ...
  const eligibilityLink = <InlineLink to={t('protected-application-intake-adult:submit.do-you-qualify.href')} className="external-link" newTabIndicator target="_blank" />;
      ...
      <div className="max-w-prose space-y-8">
        ...
        <div className="space-y-8">
          <section className="space-y-4">
            <h2 className="font-lato text-3xl leading-none font-bold">{t('protected-application-intake-adult:submit.overview')}</h2>
            <div>
              <p>{t('protected-application-intake-adult:submit.you-are-submitting')}</p>
          ...
          <section className="space-y-4">
            ...
            <ButtonLink variant="primary" routeId="protected/application/$id/intake-adult/marital-status" params={params}>
              {t('protected-application-intake-adult:submit.review-application')}
            </ButtonLink>
          </section>
          <section className="space-y-4">
            <h2 className="font-lato text-3xl leading-none font-bold">{t('protected-application-intake-adult:submit.submit-your-application')}</h2>
            <p>{t('protected-application-intake-adult:submit.by-submitting')}</p>
            <p>
              <Trans ns={handle.i18nNamespaces} i18nKey="protected-application-intake-adult:submit.review-eligibility-criteria" components={{ eligibilityLink }} />
            </p>
            <fetcher.Form method="post" noValidate>
              ...
              <div className="space-y-2">
                <InputCheckbox id="acknowledge-info" name="acknowledgeInfo" value={CHECKBOX_VALUE.yes} errorMessage={errors?.acknowledgeInfo} required>
                  {t('protected-application-intake-adult:submit.info-is-correct')}
                </InputCheckbox>
                <InputCheckbox id="acknowledge-criteria" name="acknowledgeCriteria" value={CHECKBOX_VALUE.yes} errorMessage={errors?.acknowledgeCriteria} required>
                  {t('protected-application-intake-adult:submit.i-understand')}
                </InputCheckbox>
              </div>
              <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
                <NavigationButton loading={isSubmitting} variant="primary" direction="next">
                  {t('protected-application-intake-adult:submit.submit')}
                </NavigationButton>
                <NavigationButtonLink disabled={isSubmitting} variant="secondary" direction="previous" routeId="protected/application/$id/intake-adult/dental-insurance" params={params}>
                  {t('protected-application-intake-adult:submit.dental-insurance')}
        ...
        <div className="mt-8">
          <InlineLink routeId="protected/application/$id/intake-adult/exit-application" params={params}>
            {t('protected-application-intake-adult:submit.exit-application')}
          </InlineLink>
        </div>
      </div>
      {payload && (
