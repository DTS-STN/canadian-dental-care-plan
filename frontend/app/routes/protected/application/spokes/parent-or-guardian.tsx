...
export const handle = {
  ...
  pageTitleI18nKey: 'protected-application-spokes:parentOrGuardian.pageTitle',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-spokes:parentOrGuardian.pageTitle') }) };
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  return redirect(t('protected-application-spokes:parentOrGuardian.exitBtnLink'));
}
  ...
  return (
    <div className="max-w-prose">
      <div className="mb-8 space-y-4">
        <p className="mb-4">{t('protected-application-spokes:parentOrGuardian.unableToApply')}</p>
        <p>
          <Trans ns={handle.i18nNamespaces} i18nKey="protected-application-spokes:parentOrGuardian.applyForYourself" components={{ noWrap }} />
        </p>
      </div>
      <fetcher.Form method="post" onSubmit={handleSubmit} noValidate className="flex flex-wrap items-center gap-3">
        ...
        >
          {t('protected-application-spokes:parentOrGuardian.backBtn')}
        </ButtonLink>
        <LoadingButton type="submit" variant="primary" loading={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Spoke:Exit - Parent or guardian needs to apply click">
          {t('protected-application-spokes:parentOrGuardian.exitBtn')}
        </LoadingButton>
      </fetcher.Form>
    </div>
  );
