...
export const handle = {
  i18nNamespaces: getTypedI18nNamespaces('protected-application-simplified-family', 'protected-application', 'gcweb'),
  pageIdentifier: pageIds.protected.application.simplifiedFamily.confirmation,
  pageTitleI18nKey: 'protected-application-simplified-family:confirm.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-simplified-family:confirm.page-title') }) };
...
export default function ProtectedSimplifiedFamilyConfirmation({ loaderData, params }: Route.ComponentProps) {
  ...
  const cdcpLink = <InlineLink to={t('protected-application-simplified-family:confirm.msca-link-checker')} className="external-link" newTabIndicator target="_blank" />;
    ...
    <div className="max-w-prose space-y-10">
            ...
            <DefinitionListItem term={t('confirm.dental-public')}>
              {dentalInsurance.selectedFederalBenefits || dentalInsurance.selectedProvincialBenefits ? (
                <div className="space-y-3">
                  <p>{t('protected-application-simplified-family:confirm.yes')}</p>
                  <p>{t('protected-application-simplified-family:confirm.dental-benefit-has-access')}</p>
                  <ul className="list-disc space-y-1 pl-7">
          ...
          {children.map((child) => {
            ...
            return (
              <section key={child.id} className="space-y-10">
                ...
                <div>
                  <h3 className="font-lato mb-6 text-2xl font-bold">{t('protected-application-simplified-family:confirm.page-sub-title', { child: child.firstName })}</h3>
                  <DefinitionList border>
                    <DefinitionListItem term={t('confirm.member-id')}>{child.memberId}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-family:confirm.full-name')}>{`${child.firstName} ${child.lastName}`}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-family:confirm.dob')}>{dateOfBirth}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-family:confirm.sin')}>{child.sin && formatSin(child.sin)}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-family:confirm.is-parent')}>
                      {child.isParent ? t('protected-application-simplified-family:confirm.yes') : t('protected-application-simplified-family:confirm.no')}
                    </DefinitionListItem>
                  </DefinitionList>
                </div>
                <div>
                  <h3 className="font-lato mb-6 text-2xl font-bold">{t('protected-application-simplified-family:confirm.dental-title', { child: child.firstName })}</h3>
                  <DefinitionList border>
                    <DefinitionListItem term={t('protected-application-simplified-family:confirm.dental-private')}>
                      {child.dentalInsurance.accessToDentalInsurance.hasDentalInsurance ? t('protected-application-simplified-family:confirm.yes') : t('protected-application-simplified-family:confirm.no')}
                    </DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-family:confirm.dental-public')}>
                      {child.dentalInsurance.federalBenefit.access || child.dentalInsurance.provTerrBenefit.access ? (
                        ...
                        <>{t('protected-application-simplified-family:confirm.no')}</>
                      )}
                    </DefinitionListItem>
                  </DefinitionList>
                </div>
      ...
      <Dialog>
        <DialogTrigger asChild>
          ...
          >
            {t('protected-application-simplified-family:confirm.close-application')}
          </button>
        </DialogTrigger>
        <DialogContent aria-describedby={undefined} className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t('protected-application-simplified-family:confirm.modal.header')}</DialogTitle>
          </DialogHeader>
          <p>{t('protected-application-simplified-family:confirm.modal.info')}</p>
          <p>{t('protected-application-simplified-family:confirm.modal.are-you-sure')}</p>
          <DialogFooter>
            <DialogClose asChild>
              <Button id="confirm-modal-back" variant="secondary" size="sm" data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Family:Back exit modal - Application successfully submitted click">
                {t('protected-application-simplified-family:confirm.modal.back-btn')}
              </Button>
            </DialogClose>
            <fetcher.Form method="post" noValidate>
              ...
              >
                {t('protected-application-simplified-family:confirm.modal.close-btn')}
              </Button>
            </fetcher.Form>
          </DialogFooter>
        </DialogContent>
