...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const children = await Promise.all(
    state.children.map(async (childState) => {
      ...
      const childDentalBenefits = await stateResolver.resolveChildDentalBenefitsValue({ dentalBenefits: childState.dentalBenefits }, undefined, locale);
      ...
      <div className="max-w-prose space-y-10">
        ...
        <section>
          ...
          <div className="mt-8 print:hidden">
            <PrintButton variant="primary" size="lg" errorMessage={t(($) => $.confirm.printUnavailable)} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Intake_Family:Print top - Application successfully submitted click">
              {t(($) => $.confirm.printBtn)}
            </PrintButton>
          </div>
        </section>
        ...
        <section>
          ...
          <p className="mt-4">
            <Trans ns="protectedApplicationIntakeFamily" i18nKey={($) => $.confirm.cdcpChecker} components={{ cdcpLink, noWrap: <span className="whitespace-nowrap" /> }} />
          </p>
        ...
        <section>
          ...
          <p className="mt-4">
            <Trans ns="protectedApplicationIntakeFamily" i18nKey={($) => $.confirm.getUpdatesText} components={{ mscaLinkAccount }} />
          </p>
            ...
            {children.map((child) => {
              return (
                <section key={child.id} className="space-y-10">
                  ...
                  <div>
                    <h3 className="font-lato mb-6 text-2xl font-bold">{t(($) => $.confirm.pageSubTitle, { child: child.firstName })}</h3>
                    <DefinitionList border>
        ...
        <div className="my-6">
          <div className="px-12 print:hidden">
            <PrintButton size="lg" variant="primary" errorMessage={t(($) => $.confirm.printUnavailable)} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Intake_Family:Print bottom - Application successfully submitted click">
              {t(($) => $.confirm.printBtn)}
            </PrintButton>
          </div>
        </div>
