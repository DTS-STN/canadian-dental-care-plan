...
import { loadProtectedApplicationFullFamilyStateForReview } from '~/.server/routes/helpers/protected-application-full-family-route-helpers';
...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.application.fullFamily.submit,
  pageTitleI18nKey: 'protected-application-full-family:submit.page-heading',
...
export async function loader({ context: { appContainer, session }, request, params }: Route.LoaderArgs) {
  ...
  const state = loadProtectedApplicationFullFamilyStateForReview({ params, request, session });
  validateApplicationFlow(state, params, ['full-family']);

  const t = await getFixedT(request, handle.i18nNamespaces);
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-full-family:submit.page-title') }) };
...
export async function action({ context: { appContainer, session }, request, params }: Route.ActionArgs) {
  ...
  const submitTermsSchema = z.object({
    acknowledgeInfo: z.literal(true, { error: t('protected-application-full-family:submit.error-message.acknowledge-info-required') }),
    acknowledgeCriteria: z.literal(true, { error: t('protected-application-full-family:submit.error-message.acknowledge-criteria-required') }),
  });
  ...
  return redirect(getPathById('protected/application/$id/full-family/confirmation', params));
}
...
export default function ProtectedNewFamilySubmit({ loaderData, params }: Route.ComponentProps) {
  ...
  const eligibilityLink = <InlineLink to={t('protected-application-full-family:submit.do-you-qualify.href')} className="external-link" newTabIndicator target="_blank" />;
      ...
      <div className="max-w-prose space-y-8">
        ...
        <div className="space-y-8">
          <section className="space-y-4">
            <h2 className="font-lato text-3xl leading-none font-bold">{t('protected-application-full-family:submit.overview')}</h2>
            <div>
              <p>{t('protected-application-full-family:submit.you-are-submitting')}</p>
          ...
          <section className="space-y-4">
            <h2 className="font-lato text-3xl leading-none font-bold">{t('protected-application-full-family:submit.review-your-application')}</h2>
            <p>{t('protected-application-full-family:submit.please-review')}</p>
            <ButtonLink variant="primary" routeId="protected/application/$id/full-family/marital-status" params={params}>
              {t('protected-application-full-family:submit.review-application')}
            </ButtonLink>
          </section>
          <section className="space-y-4">
            <h2 className="font-lato text-3xl leading-none font-bold">{t('protected-application-full-family:submit.submit-your-application')}</h2>
            <p>{t('protected-application-full-family:submit.by-submitting')}</p>
            <p>
              <Trans ns={handle.i18nNamespaces} i18nKey="protected-application-full-family:submit.review-eligibility-criteria" components={{ eligibilityLink }} />
            </p>
            <fetcher.Form method="post" noValidate>
              ...
              <div className="space-y-2">
                <InputCheckbox id="acknowledge-info" name="acknowledgeInfo" value={CHECKBOX_VALUE.yes} errorMessage={errors?.acknowledgeInfo} required>
                  {t('protected-application-full-family:submit.info-is-correct')}
                </InputCheckbox>
                <InputCheckbox id="acknowledge-criteria" name="acknowledgeCriteria" value={CHECKBOX_VALUE.yes} errorMessage={errors?.acknowledgeCriteria} required>
                  {t('protected-application-full-family:submit.i-understand')}
                </InputCheckbox>
              </div>
              <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
                <NavigationButton loading={isSubmitting} variant="primary" direction="next">
                  {t('protected-application-full-family:submit.submit')}
                </NavigationButton>
                <NavigationButtonLink disabled={isSubmitting} variant="secondary" direction="previous" routeId="protected/application/$id/full-family/childrens-application" params={params}>
                  {t('protected-application-full-family:submit.children-application')}
        ...
        <div className="mt-8">
          <InlineLink routeId="protected/application/$id/full-family/exit-application" params={params}>
            {t('protected-application-full-family:submit.exit-application')}
          </InlineLink>
        </div>
      </div>
      {payload && (
