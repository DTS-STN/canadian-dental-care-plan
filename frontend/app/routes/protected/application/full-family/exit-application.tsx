...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.application.fullFamily.exitApplication,
  pageTitleI18nKey: 'protected-application-full-family:exit-application.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-full-family:exit-application.page-title') }) };
  return { meta };
}
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  return redirect(t('protected-application-full-family:exit-application.exit-link'));
}
  ...
  return (
    <div className="max-w-prose">
      <div className="mb-8 space-y-4">
        <p>{t('protected-application-full-family:exit-application.are-you-sure')}</p>
        <p>{t('protected-application-full-family:exit-application.click-back')}</p>
      </div>
      <fetcher.Form method="post" noValidate className="flex flex-wrap items-center gap-3">
        ...
        <LoadingButton variant="primary" loading={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Family:Exit - Exiting the application click">
          {t('protected-application-full-family:exit-application.exit-btn')}
        </LoadingButton>
      </fetcher.Form>
    </div>
  );
