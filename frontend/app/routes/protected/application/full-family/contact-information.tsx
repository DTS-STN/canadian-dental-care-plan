...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.application.fullFamily.contactInformation,
  pageTitleI18nKey: 'protected-application-full-family:contact-information.page-heading',
...
export async function loader({ context: { appContainer, session }, request, params }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-full-family:contact-information.page-title') }) };
  ...
  return (
    <>
      ...
      <div className="max-w-prose space-y-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('protected-application-full-family:contact-information.phone-number')}</CardTitle>
            <CardAction>{sections.phoneNumber.completed && <StatusTag status="complete" />}</CardAction>
          </CardHeader>
          <CardContent>
            {state.phoneNumber?.hasChanged ? (
              <DefinitionList layout="single-column">
                <DefinitionListItem term={t('protected-application-full-family:contact-information.phone-number')}>
                  <p>{state.phoneNumber.value.primary}</p>
                </DefinitionListItem>
                {state.phoneNumber.value.alternate && (
                  <DefinitionListItem term={t('protected-application-full-family:contact-information.alt-phone-number')}>
                    <p>{state.phoneNumber.value.alternate}</p>
                  </DefinitionListItem>
                )}
              </DefinitionList>
            ) : (
              <p>{t('protected-application-full-family:contact-information.phone-number-help')}</p>
            )}
          </CardContent>
          <CardFooter className="border-t bg-zinc-100">
            <ButtonLink id="edit-button" variant="link" className="p-0" routeId="protected/application/$id/phone-number" params={params} startIcon={sections.phoneNumber.completed ? faPenToSquare : faCirclePlus} size="lg">
              {sections.phoneNumber.completed ? t('protected-application-full-family:contact-information.edit-phone-number') : t('protected-application-full-family:contact-information.add-phone-number')}
            </ButtonLink>
          </CardFooter>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('protected-application-full-family:contact-information.mailing-and-home-address')}</CardTitle>
          ...
          <CardContent>
            {mailingAddressInfo === undefined || homeAddressInfo === undefined ? (
              <p>{t('protected-application-full-family:contact-information.address-help')}</p>
            ) : (
              <DefinitionList layout="single-column">
                <DefinitionListItem term={t('protected-application-full-family:contact-information.mailing-address')}>
                  <Address
                    address={{
                ...
                <DefinitionListItem term={t('protected-application-full-family:contact-information.home-address')}>
                  <Address
                    address={{
          ...
          <CardFooter className="border-t bg-zinc-100">
            <ButtonLink id="edit-button" variant="link" className="p-0" routeId="protected/application/$id/mailing-address" params={params} startIcon={sections.address.completed ? faPenToSquare : faCirclePlus} size="lg">
              {sections.address.completed ? t('protected-application-full-family:contact-information.edit-address') : t('protected-application-full-family:contact-information.add-address')}
            </ButtonLink>
          </CardFooter>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('protected-application-full-family:contact-information.communication-preferences')}</CardTitle>
        ...
        <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
          <NavigationButtonLink disabled={!allSectionsCompleted} variant="primary" direction="next" routeId="protected/application/$id/full-family/dental-insurance" params={params}>
            {t('protected-application-full-family:contact-information.next-btn')}
          </NavigationButtonLink>
          <NavigationButtonLink variant="secondary" direction="previous" routeId="protected/application/$id/full-family/marital-status" params={params}>
            {t('protected-application-full-family:contact-information.prev-btn')}
          </NavigationButtonLink>
        </div>
      </div>
    </>
