...
export const handle = {
  ...
  pageTitleI18nKey: 'protected-application-intake-child:exit-application.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-intake-child:exit-application.page-title') }) };
  return { meta };
}
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  return redirect(t('protected-application-intake-child:exit-application.exit-link'));
}
  ...
  return (
    <div className="max-w-prose">
      <div className="mb-8 space-y-4">
        <p>{t('protected-application-intake-child:exit-application.are-you-sure')}</p>
        <p>{t('protected-application-intake-child:exit-application.click-back')}</p>
      </div>
      <fetcher.Form method="post" noValidate className="flex flex-wrap items-center gap-3">
        ...
        >
          {t('protected-application-intake-child:exit-application.back-btn')}
        </ButtonLink>
        <LoadingButton variant="primary" loading={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Children:Exit - Exiting the application click">
          {t('protected-application-intake-child:exit-application.exit-btn')}
        </LoadingButton>
      </fetcher.Form>
    </div>
  );
