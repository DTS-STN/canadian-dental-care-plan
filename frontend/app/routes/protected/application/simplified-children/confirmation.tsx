...
export const handle = {
  i18nNamespaces: getTypedI18nNamespaces('protected-application-simplified-child', 'protected-application', 'gcweb'),
  pageIdentifier: pageIds.protected.application.simplifiedChild.confirmation,
  pageTitleI18nKey: 'protected-application-simplified-child:confirm.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-application-simplified-child:confirm.page-title') }) };
...
export default function ProtectedRenewChildrenConfirmation({ loaderData, params }: Route.ComponentProps) {
  ...
  const cdcpLink = <InlineLink to={t('protected-application-simplified-child:confirm.msca-link-checker')} className="external-link" newTabIndicator target="_blank" />;
    ...
    <div className="max-w-prose space-y-10">
          ...
          {children.map((child) => {
            ...
            return (
              <section key={child.id} className="space-y-10">
                ...
                <div>
                  <h3 className="font-lato mb-6 text-2xl font-bold">{t('protected-application-simplified-child:confirm.page-sub-title', { child: child.firstName })}</h3>
                  <DefinitionList border>
                    <DefinitionListItem term={t('protected-application-simplified-child:confirm.member-id')}>{child.memberId}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-child:confirm.full-name')}>{`${child.firstName} ${child.lastName}`}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-child:confirm.dob')}>{dateOfBirth}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-child:confirm.sin')}>{child.sin && formatSin(child.sin)}</DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-child:confirm.is-parent')}>
                      {child.isParent ? t('protected-application-simplified-child:confirm.yes') : t('protected-application-simplified-child:confirm.no')}
                    </DefinitionListItem>
                  </DefinitionList>
                </div>
                <div>
                  <h3 className="font-lato mb-6 text-2xl font-bold">{t('protected-application-simplified-child:confirm.dental-title', { child: child.firstName })}</h3>
                  <DefinitionList border>
                    <DefinitionListItem term={t('protected-application-simplified-child:confirm.dental-private')}>
                      {child.dentalInsurance.accessToDentalInsurance ? t('protected-application-simplified-child:confirm.yes') : t('protected-application-simplified-child:confirm.no')}
                    </DefinitionListItem>
                    <DefinitionListItem term={t('protected-application-simplified-child:confirm.dental-public')}>
                      {child.dentalInsurance.federalBenefit.access || child.dentalInsurance.provTerrBenefit.access ? (
                        <div className="space-y-3">
                          <p>{t('protected-application-simplified-child:confirm.yes')}</p>
                          <p>{t('protected-application-simplified-child:confirm.dental-benefit-has-access')}</p>
                          <ul className="list-disc space-y-1 pl-7">
                        ...
                        <>{t('protected-application-simplified-child:confirm.no')}</>
                      )}
                    </DefinitionListItem>
                  </DefinitionList>
                </div>
      ...
      <Dialog>
        <DialogTrigger asChild>
          ...
          >
            {t('protected-application-simplified-child:confirm.close-application')}
          </button>
        </DialogTrigger>
        <DialogContent aria-describedby={undefined} className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t('protected-application-simplified-child:confirm.modal.header')}</DialogTitle>
          </DialogHeader>
          <p>{t('protected-application-simplified-child:confirm.modal.info')}</p>
          <p>{t('protected-application-simplified-child:confirm.modal.are-you-sure')}</p>
          <DialogFooter>
            <DialogClose asChild>
              <Button id="confirm-modal-back" variant="secondary" size="sm" data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Child:Back exit modal - Application successfully submitted click">
                {t('protected-application-simplified-child:confirm.modal.back-btn')}
              </Button>
            </DialogClose>
            <fetcher.Form method="post" noValidate>
              ...
              >
                {t('protected-application-simplified-child:confirm.modal.close-btn')}
              </Button>
            </fetcher.Form>
          </DialogFooter>
        </DialogContent>
