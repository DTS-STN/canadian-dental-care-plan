...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-child:review-child-information.page-title') }) };
  ...
  const children = await Promise.all(
    state.children.map(async (child) => {
      ...
      appContainer.get(TYPES.AuditService).createAudit('page-view.apply.child.review-child-information', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.child.review-child-information', { userId: idToken.sub });
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" onSubmit={handleSubmit} className="mt-6 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
          ...
          <Button
            ...
            data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Child:Exit - Review child information click"
          >
