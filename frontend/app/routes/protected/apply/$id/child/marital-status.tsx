...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const maritalStatusSchema = z.object({
    maritalStatus: z
      .string({ error: t('protected-apply-child:marital-status.error-message.marital-status-required') })
      .trim()
  ...
  const partnerInformationSchema = z.object({
    ...
    yearOfBirth: z
      ...
      .refine((year) => Number.parseInt(year) > currentYear - 150, t('protected-apply-child:marital-status.error-message.yob-is-past'))
      .refine((year) => Number.parseInt(year) < currentYear, t('protected-apply-child:marital-status.error-message.yob-is-future')),
    socialInsuranceNumber: z
      ...
      .superRefine((sin, ctx) => {
        if (!isValidSin(sin)) {
          ctx.addIssue({ code: 'custom', message: t('protected-apply-child:marital-status.error-message.sin-valid') });
        } else if (
          ...
          ctx.addIssue({ code: 'custom', message: t('protected-apply-child:marital-status.error-message.sin-unique') });
        }
      }),
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.child.marital-status', { userId: idToken.sub });
  ...
  const maritalStatusOptions = useMemo<InputRadiosProps['options']>(() => {
    return [
      ...
      },
      {
        ...
        children: t('protected-apply-child:marital-status.commonlaw'),
        ...
        <fetcher.Form method="post" noValidate>
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <LoadingButton id="cancel-button" variant="secondary" name="_action" value={FORM_ACTION.cancel} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Child:Cancel - Marital status click">
