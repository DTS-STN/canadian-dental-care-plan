...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-child:children.index.page-title') }) };
  ...
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.child.children.index', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  if (formAction === FORM_ACTION.add) {
    ...
    return redirect(getPathById('protected/apply/$id/child/children/$childId/information', { ...params, childId }));
  }
  ...
  return (
    <>
      ...
      <div className="max-w-prose">
        ...
        <p className="my-4">{t('protected-apply-child:children.index.can-apply')}</p>
        <ul className="my-4 list-disc space-y-1 pl-7">
          <li>{t('protected-apply-child:children.index.parent-guardian')}</li>
          <li>{t('protected-apply-child:children.index.over-sixteen')}</li>
          <li>{t('protected-apply-child:children.index.under-eighteen')}</li>
        </ul>
        <Collapsible className="my-4" id="name-instructions" summary={t('protected-apply-child:children.index.share-custody')}>
          <p>{t('protected-apply-child:children.index.one-application')}</p>
          <p className="my-4">{t('protected-apply-child:children.index.eligibility')}</p>
        </Collapsible>
        {children.length > 0 && (
              ...
              return (
                <section key={child.id}>
                  ...
                  <dl className="mb-6 divide-y border-y">
                    <DescriptionListItem term={t('protected-apply-child:children.index.dob-title')}>
                      <p>{dateOfBirth}</p>
                    </DescriptionListItem>
                    <DescriptionListItem term={t('protected-apply-child:children.index.sin-title')}>
                      <p>{child.information?.socialInsuranceNumber ? formatSin(child.information.socialInsuranceNumber) : ''}</p>
                    </DescriptionListItem>
                    <DescriptionListItem term={t('protected-apply-child:children.index.dental-insurance-title')}>{child.dentalInsurance ? t('protected-apply-child:children.index.yes') : t('protected-apply-child:children.index.no')}</DescriptionListItem>
                    <DescriptionListItem term={t('protected-apply-child:children.index.dental-benefit-title')}>
                      {!!child.dentalBenefits.hasFederalBenefits || !!child.dentalBenefits.hasProvincialTerritorialBenefits ? (
                        <>
                          <p>{t('protected-apply-child:children.index.yes')}</p>
                          <p>{t('protected-apply-child:children.index.dental-benefit-has-access')}</p>
                          <div>
                        ...
                        <>{t('protected-apply-child:children.index.no')}</>
                      )}
                    </DescriptionListItem>
                  </dl>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Dialog>
                      ...
                      <DialogContent aria-describedby={undefined} className="sm:max-w-md">
                        <DialogHeader>
                          <DialogTitle>{t('protected-apply-child:children.index.modal.header', { childName })}</DialogTitle>
                        </DialogHeader>
