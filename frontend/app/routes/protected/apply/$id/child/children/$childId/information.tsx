...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = {
    title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-child:children.information.page-title', { childName }) }),
    dcTermsTitle: t('gcweb:meta.title.msca-template', { title: t('protected-apply-child:children.information.page-title', { childName: childNumber }) }),
  };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.child.children.information', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const childInformationSchema = z
    .object({
      firstName: z
        .string()
        .trim()
        .min(1, t('protected-apply-child:children.information.error-message.first-name-required'))
        .max(100)
        .refine(isAllValidInputCharacters, t('protected-apply-child:children.information.error-message.characters-valid'))
        .refine((firstName) => !hasDigits(firstName), t('protected-apply-child:children.information.error-message.first-name-no-digits')),
      lastName: z
        .string()
        .trim()
        .min(1, t('protected-apply-child:children.information.error-message.last-name-required'))
        .max(100)
        .refine(isAllValidInputCharacters, t('protected-apply-child:children.information.error-message.characters-valid'))
        .refine((lastName) => !hasDigits(lastName), t('protected-apply-child:children.information.error-message.last-name-no-digits')),
      dateOfBirthYear: z.number({
        error: (issue) => (issue.input === undefined ? t('protected-apply-child:children.information.error-message.date-of-birth-year-required') : t('protected-apply-child:children.information.error-message.date-of-birth-year-number')),
      }),
      dateOfBirthMonth: z.number({
        error: (issue) => (issue.input === undefined ? t('protected-apply-child:children.information.error-message.date-of-birth-month-required') : undefined),
      }),
      dateOfBirthDay: z.number({
        error: (issue) => (issue.input === undefined ? t('protected-apply-child:children.information.error-message.date-of-birth-day-required') : t('protected-apply-child:children.information.error-message.date-of-birth-day-number')),
      }),
      dateOfBirth: z.string(),
      isParent: z.boolean({ error: t('protected-apply-child:children.information.error-message.is-parent') }),
    })
    ...
    .superRefine((val, ctx) => {
      ...
      if (!isValidDateString(dateOfBirth)) {
        ctx.addIssue({
          code: 'custom',
          message: t('protected-apply-child:children.information.error-message.date-of-birth-valid'),
          path: ['dateOfBirth'],
        });
      } else if (!isPastDateString(dateOfBirth)) {
        ctx.addIssue({
          code: 'custom',
          message: t('protected-apply-child:children.information.error-message.date-of-birth-is-past'),
          path: ['dateOfBirth'],
        });
      } else if (getAgeFromDateString(dateOfBirth) > 150) {
        ctx.addIssue({
          code: 'custom',
          message: t('protected-apply-child:children.information.error-message.date-of-birth-is-past-valid'),
          path: ['dateOfBirth'],
        });
      }
    })
  ...
  const childSinSchema = z
    .object({
      hasSocialInsuranceNumber: z.boolean({ error: t('protected-apply-child:children.information.error-message.has-social-insurance-number') }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasSocialInsuranceNumber) {
        if (!val.socialInsuranceNumber) {
          ctx.addIssue({ code: 'custom', message: t('protected-apply-child:children.information.error-message.sin-required'), path: ['socialInsuranceNumber'] });
        } else if (!isValidSin(val.socialInsuranceNumber)) {
          ctx.addIssue({ code: 'custom', message: t('protected-apply-child:children.information.error-message.sin-valid'), path: ['socialInsuranceNumber'] });
        } else if (
          ...
          ctx.addIssue({ code: 'custom', message: t('protected-apply-child:children.information.error-message.sin-unique'), path: ['socialInsuranceNumber'] });
        }
      }
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.child.children.information', { userId: idToken.sub });
  ...
  if (!parsedDataResult.data.isParent) {
    return redirect(getPathById('protected/apply/$id/child/children/$childId/parent-or-guardian', params));
  }

  if (ageCategory === 'adults' || ageCategory === 'seniors') {
    return redirect(getPathById('protected/apply/$id/child/children/$childId/cannot-apply-child', params));
  }

  return redirect(getPathById('protected/apply/$id/child/children/$childId/dental-insurance', params));
}
...
export default function ApplyFlowChildInformation({ loaderData, params }: Route.ComponentProps) {
  ...
  const options: InputRadiosProps['options'] = [
      ...
      append: hasSocialInsuranceNumberValue === true && (
        <div className="mb-6">
          <InputPatternField
            ...
            label={t('protected-apply-child:children.information.sin')}
            inputMode="numeric"
    ...
    {
      children: <Trans ns={handle.i18nNamespaces} i18nKey="protected-apply-child:children.information.sin-no" components={{ bold: <strong /> }} />,
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <div className="mb-8 space-y-6">
            <Collapsible id="name-instructions" summary={t('protected-apply-child:children.information.single-legal-name')}>
              <p>{t('protected-apply-child:children.information.name-instructions')}</p>
            </Collapsible>
            <div className="grid items-end gap-6 md:grid-cols-2">
              <InputSanitizeField
                id="first-name"
                name="firstName"
                label={t('protected-apply-child:children.information.first-name')}
                className="w-full"
                maxLength={100}
                aria-description={t('protected-apply-child:children.information.name-instructions')}
              ...
              <InputSanitizeField
                id="last-name"
                name="lastName"
                label={t('protected-apply-child:children.information.last-name')}
                className="w-full"
                maxLength={100}
            ...
            <DatePickerField
              ...
              legend={t('protected-apply-child:children.information.date-of-birth')}
              errorMessages={{
            ...
            <InputRadios id="has-social-insurance-number" legend={t('protected-apply-child:children.information.sin-legend')} name="hasSocialInsuranceNumber" options={options} errorMessage={errors?.hasSocialInsuranceNumber} required />

            <InputRadios
              id="is-parent-radios"
              name="isParent"
              legend={t('protected-apply-child:children.information.parent-legend')}
              options={[
                { value: YES_NO_OPTION.yes, children: t('protected-apply-child:children.information.radio-options.yes'), defaultChecked: defaultState?.isParent === true, readOnly: !isNew, tabIndex: isNew ? 0 : -1 },
                { value: YES_NO_OPTION.no, children: t('protected-apply-child:children.information.radio-options.no'), defaultChecked: defaultState?.isParent === false, readOnly: !isNew, tabIndex: isNew ? 0 : -1 },
              ]}
