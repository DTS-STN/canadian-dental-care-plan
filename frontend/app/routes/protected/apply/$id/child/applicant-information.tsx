...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const applicantInformationSchema = z
    .object({
      socialInsuranceNumber: z
        ...
        .superRefine((sin, ctx) => {
          if (!isValidSin(sin)) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-child:applicant-information.error-message.sin-valid') });
          } else if (
            ...
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-child:children.information.error-message.sin-unique') });
          }
        }),
      firstName: z
        .string()
      ...
      dateOfBirthYear: z.number({ required_error: t('protected-apply-child:applicant-information.error-message.date-of-birth-year-required'), invalid_type_error: t('protected-apply-child:applicant-information.error-message.date-of-birth-year-number') }),
      dateOfBirthMonth: z.number({ required_error: t('protected-apply-child:applicant-information.error-message.date-of-birth-month-required') }),
      dateOfBirthDay: z.number({ required_error: t('protected-apply-child:applicant-information.error-message.date-of-birth-day-required'), invalid_type_error: t('protected-apply-child:applicant-information.error-message.date-of-birth-day-number') }),
    ...
    .superRefine((val, ctx) => {
      ...
      if (!isValidDateString(dateOfBirth)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-child:applicant-information.error-message.date-of-birth-valid'), path: ['dateOfBirth'] });
      } else if (!isPastDateString(dateOfBirth)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-child:applicant-information.error-message.date-of-birth-is-past'), path: ['dateOfBirth'] });
      } else if (getAgeFromDateString(dateOfBirth) > 150) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-child:applicant-information.error-message.date-of-birth-is-past-valid'), path: ['dateOfBirth'] });
      }
    })
    .transform((val) => {
      // At this point the year, month and day should have been validated as positive integer
  ...
  if (!parsedDataResult.success) {
    instrumentationService.countHttpStatus('protected.apply.child.applicant-information', 400);
