...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-child:email.page-title') }) };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.child.email', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const emailSchema = z
    .object({
      email: z
        .string({ error: t('protected-apply-child:email.error-message.email-required') })
        .trim()
        .min(1)
        .max(64),
    })
    ...
    .superRefine((val, ctx) => {
      if (!validator.isEmail(val.email)) {
        ctx.addIssue({ code: 'custom', message: t('protected-apply-child:email.error-message.email-valid'), path: ['email'] });
      }
    });
  ...
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.child.email', { userId: idToken.sub });
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <ButtonLink
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Child:Cancel - Email click"
              >
