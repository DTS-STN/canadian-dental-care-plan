...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult-child:verify-email.page-title') }) };
      ...
      verificationCode: z
        ...
        .superRefine((val, ctx) => {
          if (state.verifyEmail && state.verifyEmail.verificationAttempts >= MAX_ATTEMPTS) {
            ctx.addIssue({ code: 'custom', message: t('protected-apply-adult-child:verify-email.error-message.verification-code-max-attempts'), path: ['verificationCode'] });
          }
        }),
    });
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult_Child:Save - Verify email click"
              >
