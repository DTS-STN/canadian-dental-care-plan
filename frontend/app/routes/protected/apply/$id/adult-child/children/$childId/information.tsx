...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = {
    title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult-child:children.information.page-title', { childName }) }),
    dcTermsTitle: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult-child:children.information.page-title', { childName: childNumber }) }),
  };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult-child.children.information', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const childInformationSchema = z
    .object({
      ...
      dateOfBirthYear: z.number({
        error: (issue) => (issue.input === undefined ? t('protected-apply-adult-child:children.information.error-message.date-of-birth-year-required') : t('protected-apply-adult-child:children.information.error-message.date-of-birth-year-number')),
      }),
      dateOfBirthMonth: z.number({
        error: (issue) => (issue.input === undefined ? t('protected-apply-adult-child:children.information.error-message.date-of-birth-month-required') : undefined),
      }),
      dateOfBirthDay: z.number({
        error: (issue) => (issue.input === undefined ? t('protected-apply-adult-child:children.information.error-message.date-of-birth-day-required') : t('protected-apply-adult-child:children.information.error-message.date-of-birth-day-number')),
      }),
      dateOfBirth: z.string(),
      isParent: z.boolean({ error: t('protected-apply-adult-child:children.information.error-message.is-parent') }),
    })
  ...
  const childSinSchema = z
    .object({
      hasSocialInsuranceNumber: z.boolean({ error: t('protected-apply-adult-child:children.information.error-message.has-social-insurance-number') }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasSocialInsuranceNumber) {
        if (!val.socialInsuranceNumber) {
          ctx.addIssue({ code: 'custom', message: t('protected-apply-adult-child:children.information.error-message.sin-required'), path: ['socialInsuranceNumber'] });
        } else if (!isValidSin(val.socialInsuranceNumber)) {
          ctx.addIssue({ code: 'custom', message: t('protected-apply-adult-child:children.information.error-message.sin-valid'), path: ['socialInsuranceNumber'] });
        } else if (
          ...
          ctx.addIssue({ code: 'custom', message: t('protected-apply-adult-child:children.information.error-message.sin-unique'), path: ['socialInsuranceNumber'] });
        }
      }
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult-child.children.information', { userId: idToken.sub });
