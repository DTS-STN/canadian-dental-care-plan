...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.apply.adultChild.childConfirmFederalProvincialTerritorialBenefits,
  pageTitleI18nKey: 'protected-apply-adult-child:children.confirm-dental-benefits.title',
};
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = {
    title: t('gcweb:meta.title.template', { title: t('protected-apply-adult-child:children.confirm-dental-benefits.title', { childName }) }),
    dcTermsTitle: t('gcweb:meta.title.template', { title: t('protected-apply-adult-child:children.confirm-dental-benefits.title', { childName: childNumber }) }),
  };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.domain.services.AuditService).createAudit('page-view.apply.adult-child.children.confirm-federal-provincial-territorial-benefits', { userId: idToken.sub });

  instrumentationService.countHttpStatus('protected.apply.adult-child.children.confirm-federal-provincial-territorial-benefits', 200);
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const dentalBenefitsChangedSchema = z.object({
    hasFederalProvincialTerritorialBenefits: z.boolean({ errorMap: () => ({ message: t('protected-apply-adult-child:children.confirm-dental-benefits.error-message.federal-provincial-territorial-benefit-required') }) }),
  });
  ...
  if (!parsedDentalBenefitsResult.success) {
    instrumentationService.countHttpStatus('protected.apply.adult-child.children.confirm-federal-provincial-territorial-benefits', 400);
    return data(
      {
        errors: {
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.domain.services.AuditService).createAudit('update-data.apply.adult-child.children.confirm-federal-provincial-territorial-benefits', { userId: idToken.sub });

  instrumentationService.countHttpStatus('protected.apply.adult-child.children.confirm-federal-provincial-territorial-benefits', 302);

  if (dentalBenefits.hasFederalProvincialTerritorialBenefits === true) {
    return redirect(getPathById('protected/apply/$id/adult-child/children/$childId/federal-provincial-territorial-benefits', params));
  }
  ...
  return (
    <>
      ...
      <div className="max-w-prose">
        <p className="mb-4">{t('protected-apply-adult-child:children.confirm-dental-benefits.access-to-dental')}</p>
        <p className="mb-4">{t('protected-apply-adult-child:children.confirm-dental-benefits.eligibility-criteria')}</p>
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <fieldset className="mb-6">
            <InputRadios
              ...
              legend={t('protected-apply-adult-child:children.confirm-dental-benefits.has-benefits', { childName })}
              options={[
                {
                  children: <Trans ns={handle.i18nNamespaces} i18nKey="protected-apply-adult-child:children.confirm-dental-benefits.option-yes" />,
                ...
                {
                  children: <Trans ns={handle.i18nNamespaces} i18nKey="protected-apply-adult-child:children.confirm-dental-benefits.option-no" />,
          ...
          {editMode ? (
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Button id="save-button" variant="primary" disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult_Child:Save - Child access to other government dental benefits click">
                {t('protected-apply-adult-child:children.confirm-dental-benefits.button.save-btn')}
              </Button>
              <ButtonLink
                id="cancel-button"
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult_Child:Cancel - Child access to other government dental benefits click"
              >
                {t('protected-apply-adult-child:children.confirm-dental-benefits.button.cancel-btn')}
              </ButtonLink>
            </div>
          ) : (
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult_Child:Continue - Child access to other government dental benefits click"
              >
                {t('protected-apply-adult-child:children.confirm-dental-benefits.button.continue')}
              </LoadingButton>
              <ButtonLink
                id="back-button"
                routeId="protected/apply/$id/adult-child/children/$childId/dental-insurance"
                params={params}
                disabled={isSubmitting}
                startIcon={faChevronLeft}
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult_Child:Back - Child access to other government dental benefits click"
              >
                {t('protected-apply-adult-child:children.confirm-dental-benefits.button.back')}
              </ButtonLink>
            </div>
          )}
        </fetcher.Form>
