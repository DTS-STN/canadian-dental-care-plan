...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = {
    title: t('gcweb:meta.title.template', { title: t('protected-apply-adult-child:children.dental-benefits.title', { childName }) }),
    dcTermsTitle: t('gcweb:meta.title.template', { title: t('protected-apply-adult-child:children.dental-benefits.title', { childName: childNumber }) }),
  };

  instrumentationService.countHttpStatus('protected.apply.adult-child.children.federal-provincial-territorial-benefits', 200);
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  if (formAction === FORM_ACTION.cancel) {
    ...
    instrumentationService.countHttpStatus('protected.apply.adult-child.children.federal-provincial-territorial-benefits', 302);
  ...
  const federalBenefitsSchema = z
    .object({
      hasFederalBenefits: z.boolean({ errorMap: () => ({ message: t('protected-apply-adult-child:children.dental-benefits.error-message.federal-benefit-required') }) }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasFederalBenefits) {
        if (!val.federalSocialProgram || validator.isEmpty(val.federalSocialProgram)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-adult-child:children.dental-benefits.error-message.federal-benefit-program-required'), path: ['federalSocialProgram'] });
        }
      }
    })
    .transform((val) => {
  ...
  const provincialTerritorialBenefitsSchema = z
    .object({
      hasProvincialTerritorialBenefits: z.boolean({ errorMap: () => ({ message: t('protected-apply-adult-child:children.dental-benefits.error-message.provincial-benefit-required') }) }),
    ...
    .superRefine((val, ctx) => {
      if (val.hasProvincialTerritorialBenefits) {
        if (!val.province || validator.isEmpty(val.province)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-adult-child:children.dental-benefits.error-message.provincial-territorial-required'), path: ['province'] });
        } else if (!val.provincialTerritorialSocialProgram || validator.isEmpty(val.provincialTerritorialSocialProgram)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-adult-child:children.dental-benefits.error-message.provincial-benefit-program-required'), path: ['provincialTerritorialSocialProgram'] });
        }
      }
    })
    .transform((val) => {
  ...
  if (!parsedFederalBenefitsResult.success || !parsedProvincialTerritorialBenefitsResult.success) {
    instrumentationService.countHttpStatus('protected.apply.adult-child.children.federal-provincial-territorial-benefits', 400);
    return data(
      {
        errors: {
