...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.apply.adultChild.phoneNumber,
  pageTitleI18nKey: 'protected-apply-adult-child:phone-number.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.template', { title: t('protected-apply-adult-child:phone-number.page-title') }) };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.domain.services.AuditService).createAudit('page-view.apply.adult-child.phone-number', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const phoneNumberSchema = z.object({
    phoneNumber: phoneSchema({
      invalid_phone_canadian_error: t('protected-apply-adult-child:phone-number.error-message.phone-number-valid'),
      invalid_phone_international_error: t('protected-apply-adult-child:phone-number.error-message.phone-number-valid-international'),
    }).optional(),
    phoneNumberAlt: phoneSchema({
      invalid_phone_canadian_error: t('protected-apply-adult-child:phone-number.error-message.phone-number-alt-valid'),
      invalid_phone_international_error: t('protected-apply-adult-child:phone-number.error-message.phone-number-alt-valid-international'),
    }).optional(),
  });
  ...
  if (!parsedDataResult.success) {
    instrumentationService.countHttpStatus('protected.apply.adult-child.phone-number', 400);
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.domain.services.AuditService).createAudit('update-data.apply.adult-child.phone-number', { userId: idToken.sub });

  instrumentationService.countHttpStatus('protected.apply.adult-child.phone-number', 302);

  if (state.editMode) {
    return redirect(getPathById('protected/apply/$id/adult-child/review-adult-information', params));
  }

  return redirect(getPathById('protected/apply/$id/adult-child/communication-preference', params));
}
  ...
  return (
    <>
      <div className="my-6 sm:my-8">
        <Progress value={52} size="lg" label={t('protected-apply:progress.label')} />
      </div>
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <div className="mb-6">
            <p className="mb-4">{t('protected-apply-adult-child:phone-number.help-message')}</p>
            <div className="grid items-end gap-6">
              <InputPhoneField
                ...
                label={t('protected-apply-adult-child:phone-number.phone-number')}
                maxLength={100}
              ...
              <InputPhoneField
                ...
                label={t('protected-apply-adult-child:phone-number.phone-number-alt')}
                maxLength={100}
                aria-describedby="adding-phone"
              />
            </div>
          ...
          {editMode ? (
            ...
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              ...
              <ButtonLink
                id="back-button"
                routeId={defaultState.backToMailingAddress ? 'protected/apply/$id/adult-child/mailing-address' : 'protected/apply/$id/adult-child/home-address'}
                params={params}
                disabled={isSubmitting}
