...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult-child:parent-or-guardian.page-title') }) };
  ...
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult-child.parent-or-guardian', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult-child.parent-or-guardian', { userId: idToken.sub });
  ...
  return (
    <div className="max-w-prose">
      ...
      <fetcher.Form method="post" onSubmit={handleSubmit} noValidate className="flex flex-wrap items-center gap-3">
        ...
        <LoadingButton type="submit" variant="primary" loading={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult_Child:Return to my dashboard - Parent or legal guardian needs to apply click">
