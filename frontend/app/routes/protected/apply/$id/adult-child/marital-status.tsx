...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult-child:marital-status.page-title') }) };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult-child.marital-status', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const maritalStatusSchema = z.object({
    maritalStatus: z
      .string({ error: t('protected-apply-adult-child:marital-status.error-message.marital-status-required') })
      .trim()
  ...
  const partnerInformationSchema = z.object({
    ...
    socialInsuranceNumber: z
      ...
      .superRefine((sin, ctx) => {
        if (!isValidSin(sin)) {
          ctx.addIssue({ code: 'custom', message: t('protected-apply-adult-child:marital-status.error-message.sin-valid') });
        } else if (
          ...
          ctx.addIssue({ code: 'custom', message: t('protected-apply-adult-child:marital-status.error-message.sin-unique') });
        }
      }),
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult-child.marital-status', { userId: idToken.sub });
  ...
  const maritalStatusOptions = useMemo<InputRadiosProps['options']>(() => {
    return [
      ...
      },
      {
        ...
        children: t('protected-apply-adult-child:marital-status.commonlaw'),
          ...
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult_Child:Cancel - Marital status click"
              >
