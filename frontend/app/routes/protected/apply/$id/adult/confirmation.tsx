...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult.confirmation', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult.confirmation', { userId: idToken.sub });
      ...
      <section>
        <ContextualAlert type="comment">
          <div className="space-y-4">
            ...
            <ButtonLink id="survey-button" to={surveyLink} params={params} target="_blank" data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Confirmation survey button - Take the survey click" variant="primary">
      ...
      <section className="space-y-8">
        ...
        <section className="space-y-6">
          ...
          <dl className="divide-y border-y">
            ...
            <DescriptionListItem term={t('confirm.marital-status')}>{userInfo.maritalStatus ? t(`protected-apply-adult:${maritalStatusMap[userInfo.maritalStatus as keyof typeof maritalStatusMap]}`) : ''}</DescriptionListItem>
