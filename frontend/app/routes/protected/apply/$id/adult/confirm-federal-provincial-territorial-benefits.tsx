...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult:confirm-dental-benefits.title') }) };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult.confirm-federal-provincial-territorial-benefits', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const dentalBenefitsChangedSchema = z.object({
    hasFederalProvincialTerritorialBenefits: z.boolean({ error: t('protected-apply-adult:confirm-dental-benefits.error-message.federal-provincial-territorial-benefit-required') }),
  });
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult.confirm-federal-provincial-territorial-benefits', { userId: idToken.sub });
