...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult.dental-insurance', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const dentalInsuranceSchema = z.object({
    dentalInsurance: z.boolean({ error: t('protected-apply-adult:dental-insurance.error-message.dental-insurance-required') }),
  });
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult.dental-insurance', { userId: idToken.sub });
