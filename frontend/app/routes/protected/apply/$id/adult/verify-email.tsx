...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.apply.adult.verifyEmail,
  pageTitleI18nKey: 'protected-apply-adult:verify-email.page-title',
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  if (formAction === FORM_ACTION.request) {
    ...
    if (state.editMode) {
      ...
      instrumentationService.countHttpStatus('protected.apply.adult.verify-email.verification-code-sent', 200);
  ...
  if (formAction === FORM_ACTION.submit) {
      ...
      verificationCode: z
        ...
        .superRefine((val, ctx) => {
          if (state.verifyEmail && state.verifyEmail.verificationAttempts >= MAX_ATTEMPTS) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: t('protected-apply-adult:verify-email.error-message.verification-code-max-attempts'), path: ['verificationCode'] });
          }
        }),
    });
    ...
    if (!parsedDataResult.success) {
      instrumentationService.countHttpStatus('protected.apply.adult.verify-email', 400);
    ...
    if (state.verifyEmail && state.verifyEmail.verificationCode !== parsedDataResult.data.verificationCode) {
      ...
      instrumentationService.countHttpStatus('protected.apply.adult.verify-email.verification-code-mismatch', 200);
      return { status: 'verification-code-mismatch' } as const;
    }

    instrumentationService.countHttpStatus('protected.apply.adult.verify-email', 302);
...
export default function ProtectedApplyFlowVerifyEmail({ loaderData, params }: Route.ComponentProps) {
  ...
  const communicationLink = <InlineLink routeId="protected/apply/$id/adult/communication-preference" params={params} />;
  ...
  return (
    <>
      ...
      <div className="max-w-prose">
        <ErrorAlert>
          <h2 className="mb-2 font-bold">{t('protected-apply-adult:verify-email.verification-code-alert.heading')}</h2>
          <p className="mb-2">{t('protected-apply-adult:verify-email.verification-code-alert.detail')}</p>
        </ErrorAlert>
        <errorSummary.ErrorSummary />
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <fieldset className="mb-6">
            <p className="mb-4">{t('protected-apply-adult:verify-email.verification-code', { email })}</p>
            <p className="mb-4">{t('protected-apply-adult:verify-email.request-new')}</p>
            <p className="mb-8">
              <Trans ns={handle.i18nNamespaces} i18nKey="protected-apply-adult:verify-email.unable-to-verify" components={{ communicationLink }} />
            </p>
            ...
            <div className="grid items-end gap-6 md:grid-cols-2">
              <InputField
                ...
                label={t('protected-apply-adult:verify-email.verification-code-label')}
            ...
            >
              {t('protected-apply-adult:verify-email.request-new-code')}
            </LoadingButton>
          </fieldset>
          {editMode ? (
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('protected-apply-adult:verify-email.code-sent.heading')}</DialogTitle>
            </DialogHeader>
            <DialogDescription>{t('protected-apply-adult:verify-email.code-sent.detail', { email })}</DialogDescription>
            <DialogFooter>
              <DialogClose asChild>
