...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.apply.adult.maritalStatus,
  pageTitleI18nKey: 'protected-apply-adult:marital-status.page-title',
...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult:marital-status.page-title') }) };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult.marital-status', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const partnerInformationSchema = z.object({
    ...
    yearOfBirth: z
      ...
      .refine((year) => Number.parseInt(year) > currentYear - 150, t('protected-apply-adult:marital-status.error-message.yob-is-past'))
      .refine((year) => Number.parseInt(year) < currentYear, t('protected-apply-adult:marital-status.error-message.yob-is-future')),
  ...
  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult.marital-status', { userId: idToken.sub });

  if (state.editMode) {
    return redirect(getPathById('protected/apply/$id/adult/review-information', params));
  }

  return redirect(getPathById('protected/apply/$id/adult/mailing-address', params));
}

export default function ProtectedApplyAdultMaritalStatus({ loaderData, params }: Route.ComponentProps) {
    ...
    return [
      {
        ...
        children: t('protected-apply-adult:marital-status.single'),
        defaultChecked: defaultState.maritalStatus === MARITAL_STATUS.single,
        onChange: handleChange,
      },
      {
        value: MARITAL_STATUS.married,
        children: t('protected-apply-adult:marital-status.married'),
  ...
  return (
    <>
      <div className="my-6 sm:my-8">
        <Progress value={40} size="lg" label={t('protected-apply:progress.label')} />
      </div>
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          ...
          <div className="mb-8 space-y-6">
            <InputRadios id="marital-status" name="maritalStatus" legend={t('protected-apply-adult:marital-status.marital-status')} options={maritalStatusOptions} errorMessage={errors?.maritalStatus} required />

            {(marriedOrCommonlaw === MARITAL_STATUS.commonlaw || marriedOrCommonlaw === MARITAL_STATUS.married) && (
              <>
                <h2 className="font-lato mb-6 text-2xl font-bold">{t('protected-apply-adult:marital-status.spouse-or-commonlaw')}</h2>
                <p className="mb-4">{t('protected-apply-adult:marital-status.provide-sin')}</p>
                <p className="mb-6">{t('protected-apply-adult:marital-status.required-information')}</p>
                <InputPatternField
                  ...
                  inputMode="numeric"
                  helpMessagePrimary={t('protected-apply-adult:marital-status.help-message.sin')}
                ...
                <InputPatternField
                  ...
                  label={t('protected-apply-adult:marital-status.year-of-birth')}
                ...
                <InputCheckbox id="confirm" name="confirm" value="yes" errorMessage={errors?.confirm} defaultChecked={defaultState.confirm === true} required>
                  {t('protected-apply-adult:marital-status.confirm-checkbox')}
                </InputCheckbox>
              </>
            )}
          </div>
          {editMode ? (
            <div className="flex flex-wrap items-center gap-3">
              <Button id="save-button" name="_action" value={FORM_ACTION.save} variant="primary" disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Save - Marital status click">
                {t('protected-apply-adult:marital-status.save-btn')}
              </Button>
              <LoadingButton id="cancel-button" variant="secondary" name="_action" value={FORM_ACTION.cancel} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Cancel - Marital status click">
                {t('protected-apply-adult:marital-status.cancel-btn')}
              </LoadingButton>
            </div>
          ) : (
            <div className="flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Continue - Marital status click"
              >
                {t('protected-apply-adult:marital-status.continue-btn')}
              </LoadingButton>
              <ButtonLink
                id="back-button"
                variant="secondary"
                routeId={isNewUser ? 'protected/apply/$id/adult/new-or-existing-member' : 'protected/apply/$id/adult/applicant-information'}
                params={params}
                disabled={isSubmitting}
                startIcon={faChevronLeft}
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Back - Marital status click"
              >
                {t('protected-apply-adult:marital-status.back-btn')}
              </ButtonLink>
            </div>
          )}
        </fetcher.Form>
