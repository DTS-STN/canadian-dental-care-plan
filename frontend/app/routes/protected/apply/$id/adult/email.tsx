...
export async function loader({ context: { appContainer, session }, params, request }: Route.LoaderArgs) {
  ...
  const meta = { title: t('gcweb:meta.title.msca-template', { title: t('protected-apply-adult:email.page-title') }) };

  const idToken: IdToken = session.get('idToken');
  appContainer.get(TYPES.AuditService).createAudit('page-view.apply.adult.email', { userId: idToken.sub });
...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const emailSchema = z
    .object({
      email: z
        .string({ error: t('protected-apply-adult:email.error-message.email-required') })
        .trim()
        .min(1)
        .max(64),
    })
  ...
  appContainer.get(TYPES.AuditService).createAudit('update-data.apply.adult.email', { userId: idToken.sub });
