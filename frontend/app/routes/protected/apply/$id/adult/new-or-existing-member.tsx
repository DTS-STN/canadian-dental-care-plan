...
export const handle = {
  ...
  pageIdentifier: pageIds.protected.apply.adult.newOrExistingMember,
  pageTitleI18nKey: 'protected-apply-adult:new-or-existing-member.page-title',
  ...
  const newOrExistingMemberSchema = z
    .object({
      newOrExistingMember: z.enum(NEW_OR_EXISTING_MEMBER_OPTION, {
        error: t('protected-apply-adult:new-or-existing-member.error-message.is-new-or-existing-member-required'),
      }),
      clientNumber: z
        .string()
        .trim()
    ...
    .superRefine((val, ctx) => {
      if (val.newOrExistingMember === NEW_OR_EXISTING_MEMBER_OPTION.yes) {
        if (!val.clientNumber) {
          ctx.addIssue({
            code: 'custom',
            message: t('protected-apply-adult:new-or-existing-member.error-message.client-number-required'),
            path: ['clientNumber'],
          });
        } else if (!isValidClientNumberRenewal(val.clientNumber)) {
          ctx.addIssue({
            code: 'custom',
            message: t('protected-apply-adult:new-or-existing-member.error-message.client-number-valid'),
            path: ['clientNumber'],
          });
        }
      }
...
export default function ProtectedApplyFlowNewOrExistingMember({ loaderData, params }: Route.ComponentProps) {
  ...
  const options: InputRadiosProps['options'] = [
    {
      children: <Trans ns={handle.i18nNamespaces} i18nKey="protected-apply-adult:new-or-existing-member.yes" components={{ bold: <strong /> }} />,
    ...
    {
      children: <Trans ns={handle.i18nNamespaces} i18nKey="protected-apply-adult:new-or-existing-member.no" components={{ bold: <strong /> }} />,
    ...
    <>
      ...
      <div className="max-w-prose">
        ...
        <fetcher.Form method="post" noValidate>
          <CsrfTokenInput />
          <InputRadios id="new-or-existing-member" name="newOrExistingMember" legend={t('protected-apply-adult:new-or-existing-member.previously-enrolled')} options={options} errorMessage={errors?.newOrExistingMember} required />
          {isNewOrExistingMember && (
            <div className="my-8">
              <InputPatternField
                ...
                label={t('protected-apply-adult:new-or-existing-member.client-number-label')}
                inputMode="numeric"
                defaultValue={defaultState?.clientNumber ?? ''}
                errorMessage={errors?.clientNumber}
                helpMessagePrimary={t('protected-apply-adult:new-or-existing-member.client-number-description')}
          ...
          {editMode ? (
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <Button id="save-button" name="_action" value={FORM_ACTION.save} variant="primary" disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Save - New or existing member click">
                {t('protected-apply-adult:new-or-existing-member.save-btn')}
              </Button>
              <Button id="cancel-button" name="_action" variant="secondary" value={FORM_ACTION.cancel} disabled={isSubmitting} data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Cancel - New or existing member click">
                {t('protected-apply-adult:new-or-existing-member.cancel-btn')}
              </Button>
            </div>
          ) : (
            <div className="mt-8 flex flex-row-reverse flex-wrap items-center justify-end gap-3">
              <LoadingButton
                ...
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Continue - New or existing member click"
              >
                {t('protected-apply-adult:new-or-existing-member.continue-btn')}
              </LoadingButton>
              <ButtonLink
                id="back-button"
                variant="secondary"
                routeId={userAgeCategory === 'youth' ? 'protected/apply/$id/adult/living-independently' : 'protected/apply/$id/adult/applicant-information'}
                params={params}
                disabled={isSubmitting}
                startIcon={faChevronLeft}
                data-gc-analytics-customclick="ESDC-EDSC:CDCP Online Application Form-Protected-Adult:Back - New or existing member click"
              >
                {t('protected-apply-adult:new-or-existing-member.back-btn')}
              </ButtonLink>
            </div>
          )}
        </fetcher.Form>
