...
export async function action({ context: { appContainer, session }, params, request }: Route.ActionArgs) {
  ...
  const livingIndependentlySchema = z.object({
    livingIndependently: z.nativeEnum(LIVING_INDEPENDENTLY_OPTION, {
      errorMap: () => ({ message: t('protected-apply-adult:living-independently.error-message.living-independently-required') }),
    }),
  });
  ...
  if (!parsedDataResult.success) {
    instrumentationService.countHttpStatus('protected.apply.adult.living-independently', 400);
