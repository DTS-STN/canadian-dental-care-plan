...
export function validateApplyChildStateForReview({ params, state }: ValidateStateForReviewArgs) {
  const { applicantInformation, applicationYear, communicationPreferences, dateOfBirth, maritalStatus, editMode, id, lastUpdatedOn, partnerInformation, mailingAddress, homeAddress, contactInformation, submissionInfo, taxFiling2023, typeOfApplication } =
    state;
