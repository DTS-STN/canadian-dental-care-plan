...
export function loadProtectedApplyState({ params, session }: LoadStateArgs) {
  const log = getLogger('protected-apply-route-helpers.server/loadProtectedApplyState');
...
interface SaveStateArgs {
  params: ProtectedApplyStateParams;
  session: Session;
  state: Partial<OmitStrict<ProtectedApplyState, 'id' | 'lastUpdatedOn'>>;
  remove?: keyof OmitStrict<ProtectedApplyState, 'children' | 'editMode' | 'id' | 'lastUpdatedOn'>;
}
...
export function saveProtectedApplyState({ params, session, state, remove = undefined }: SaveStateArgs) {
  const log = getLogger('protected-apply-route-helpers.server/saveApplyState');
...
export function clearProtectedApplyState({ params, session }: ClearStateArgs) {
  const log = getLogger('protected-apply-route-helpers.server/clearProtectedApplyState');
 ...
 */
export function startProtectedApplyState({ id, session }: StartArgs) {
  const log = getLogger('protected-apply-route-helpers.server/startApplyState');
