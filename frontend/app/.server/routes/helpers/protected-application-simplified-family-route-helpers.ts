...
/**
 * Loads protected application simplified family state.
...
export function loadProtectedApplicationSimplifiedFamilyState({ params, request, session }: LoadProtectedApplicationSimplifiedFamilyStateArgs) {
  ...
  const confirmationRouteUrl = getPathById('protected/application/$id/simplified-family/confirmation', params);
 ...
 */
export function loadProtectedApplicationSimplifiedFamilyStateForReview({ params, request, session }: LoadProtectedApplicationSimplifiedFamilyStateForReviewArgs) {
...
export function validateProtectedApplicationFamilyStateForReview({ params, state }: ValidateProtectedApplicationFamilyStateForReviewArgs) {
  ...
  if (applicantInformationStateHasPartner(maritalStatus) && !partnerInformation) {
    throw redirect(getPathById('protected/application/$id/simplified-family/marital-status', params));
  }
  ...
  if (dentalInsurance === undefined) {
    throw redirect(getPathById('protected/application/$id/simplified-family/dental-insurance', params));
  }
...
function validateChildrenStateForReview({ childrenState, params }: ValidateChildrenStateForReviewArgs) {
  ...
  if (children.length === 0) {
    throw redirect(getPathById('protected/application/$id/simplified-family/childrens-application', params));
  }
