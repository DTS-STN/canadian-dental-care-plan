...
export class DefaultRedisService implements RedisService {
  ...
  private singletonRedisClient: RedisClientType | RedisSentinelType | undefined;

  constructor() {
    this.log = createLogger('DefaultRedisService');
  }

  private async getRedisClient(): Promise<RedisClientType | RedisSentinelType> {
    if (this.singletonRedisClient) {
      return this.singletonRedisClient;
    }

    this.singletonRedisClient = await getRedisClient();
    return this.singletonRedisClient;
  }
