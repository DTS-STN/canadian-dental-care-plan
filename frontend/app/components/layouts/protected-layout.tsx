      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent onFocusOutside={(e) => e.preventDefault()} side="bottom" avoidCollisions={false} className="w-svw rounded-t-none sm:w-[260px]" sideOffset={0} align="center">
          <DropdownMenuItem asChild className="cursor-pointer">
            <Link to={t('gcweb:header.menuDashboardHref', { baseUri: SCCH_BASE_URI })} data-gc-analytics-customclick="ESDC-EDSC_MSCA-MSDC-SCH:Nav Menu:my-dashboard">
              {t('gcweb:header.menuDashboard.text')}
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild className="cursor-pointer">
            <Link to={t('gcweb:header.menuProfileHref', { baseUri: SCCH_BASE_URI })} data-gc-analytics-customclick="ESDC-EDSC_MSCA-MSDC-SCH:Nav Menu:profile">
              {t('gcweb:header.menuProfile.text')}
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild className="cursor-pointer">
            <Link to={t('gcweb:header.menuSecuritySettingsHref', { baseUri: SCCH_BASE_URI })} data-gc-analytics-customclick="ESDC-EDSC_MSCA-MSDC-SCH:Nav Menu:security">
              {t('gcweb:header.menuSecuritySettings.text')}
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild className="cursor-pointer">
            <Link to={t('gcweb:header.menuContactUsHref', { baseUri: SCCH_BASE_URI })} data-gc-analytics-customclick="ESDC-EDSC_MSCA-MSDC-SCH:Nav Menu:contact">
              {t('gcweb:header.menuContactUs.text')}
            </Link>
          </DropdownMenuItem>
          ...
          <DropdownMenuItem asChild className="cursor-pointer">
            ...
            >
              {t('gcweb:header.menuSignOut.text')} <FontAwesomeIcon icon={faArrowRightFromBracket} className="size-4 shrink-0" />
            </Link>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
        ...
        <div className="sm:container">
          <div className="flex flex-col items-stretch justify-between sm:flex-row sm:items-center">
            <h2 className="font-lato p-4 text-xl font-semibold sm:p-0 sm:py-3 sm:text-2xl">
              <AppLink to={t('gcweb:header.menuDashboardHref', { baseUri: SCCH_BASE_URI })} className="hover:underline">
                {t('gcweb:header.applicationTitleMsca')}
              </AppLink>
            </h2>
            <div className="items-center gap-8 md:flex">
              <ButtonLink
                ...
                to={t('gcweb:header.menuInboxHref', { baseUri: SCCH_BASE_URI })}
                data-gc-analytics-customclick="ESDC-EDSC_MSCA-MSDC-SCH:Nav Menu:my-dashboard"
              >
                {t('gcweb:header.menuInbox.text')}
              </ButtonLink>
              <NavigationMenu />
            </div>
          </div>
...
function PageBreadcrumbs() {
  ...
  return (
    <Breadcrumbs
      className="my-4"
      items={[
        { content: t('gcweb:breadcrumbs.dashboard'), to: t('gcweb:header.menuDashboardHref', { baseUri: SCCH_BASE_URI }) },
    ...
    <footer id="wb-info" tabIndex={-1} className="bg-stone-50 print:hidden">
      <div className="bg-gray-700 text-white">
        <section className="container py-6">
          <h2 className="mb-4">{t('gcweb:footer.mainBand.header.protected')}</h2>
          <div className="grid gap-x-4 gap-y-2 text-sm sm:grid-cols-3">
            <Link className="hover:underline" to={t('gcweb:footer.mainBand.links.contactUs.protectedHref', { baseUri: SCCH_BASE_URI })} data-gc-analytics-navigation={`Footer:Footer:${t('gcweb:footer.mainBand.links.contactUs.content')}`}>
        ...
        <div className="flex items-center justify-between gap-4">
          <nav aria-labelledby="gc-corporate">
            ...
            <div className="flex flex-col items-start gap-2 text-sm leading-6 sm:flex-row sm:items-center sm:gap-4">
              <Link className="text-slate-700 hover:underline" to={t('gcweb:footer.termsConditions.protectedHref', { baseUri: ECAS_BASE_URI })} data-gc-analytics-navigation={`Footer:Footer:${t('gcweb:footer.termsConditions.text')}`}>
                {t('gcweb:footer.termsConditions.text')}
              </Link>
              <div className="hidden size-0 rounded-full border-[3px] border-slate-700 sm:block"></div>
              <Link className="text-slate-700 hover:underline" to={t('gcweb:footer.privacy.protectedHref', { baseUri: ECAS_BASE_URI })} data-gc-analytics-navigation={`Footer:Footer:${t('gcweb:footer.privacy.text')}`}>
...
export function NotFoundError({ error }: NotFoundErrorProps) {
  const { t } = useTranslation(i18nNamespaces);
  const { SCCH_BASE_URI } = getClientEnv();
  const dashboard = <InlineLink to={t('gcweb:header.menuDashboardHref', { baseUri: SCCH_BASE_URI })} />;
    ...
    <>
      ...
      <main className="container" property="mainContentOfPage" resource="#wb-main" typeof="WebPageElement">
        <PageTitle className="my-8">
          <span>{t('gcweb:protectedNotFound.pageTitle')}</span>
          <small className="block text-2xl font-normal text-neutral-500">{t('gcweb:protectedNotFound.pageSubtitle')}</small>
        </PageTitle>
        <p className="mb-8 text-lg text-gray-500">{t('gcweb:protectedNotFound.pageMessage')}</p>
        <ul className="list-disc space-y-2 pl-10">
          <li>
            <Trans ns={i18nNamespaces} i18nKey="gcweb:protectedNotFound.pageLink" components={{ dashboard }} />
          </li>
        </ul>
        <PageDetails />
      </main>
