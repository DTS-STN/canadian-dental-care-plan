...
@Service
@CacheConfig(cacheNames = { "alert-type" })
public class AlertTypeService {
	private final AlertTypeMapper mapper;

	private final AlertTypeRepository repository;

	public AlertTypeService(@Lazy AlertTypeMapper mapper, AlertTypeRepository repository) {
	...
	@Cacheable(key = "{ 'id', #id }")
	public Optional<AlertType> read(String id) {
