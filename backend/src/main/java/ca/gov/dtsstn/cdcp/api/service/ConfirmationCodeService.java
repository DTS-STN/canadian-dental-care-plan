...
public class ConfirmationCodeService {
    ...
    public ConfirmationCodeStatus verifyConfirmationCode(ConfirmationCode confirmationCode, String userId){
        if (confirmationCode == null || confirmationCode.getCode().isBlank()) {
            return ConfirmationCodeStatus.NO_CODE;
        }
    	if (!getConfirmationCodesByUserId(userId).contains(confirmationCode)) {
